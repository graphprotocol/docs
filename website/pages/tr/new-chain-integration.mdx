...
Eğer farklı bir zincir türüyle ilgileniyorsanız, Graph Düğümü ile yeni bir entegrasyon oluşturmanız gerekecektir. Bizim önerdiğimiz yöntem, zincir için bir Firehose geliştirilmesi ve ardından bu Firehose'un Graph Düğümü ile entegrasyonudur.
...
EVM tabanlı olmayan zincirler için, Graph Düğümü blok zinciri verilerini gRPC ve bilinen tip tanımları aracılığıyla alması gerekecektir. Bu, [StreamingFast] \(https://www.streamingfast.io/) tarafından geliştirilen ve dosya tabanlı ve akış öncelikli bir yaklaşım kullanarak yüksek ölçeklenebilir bir indeksleme blok zinciri çözümü sağlayan yeni bir teknoloji olan [Firehose] \(firehose/README/) aracılığıyla yapılabilir.
