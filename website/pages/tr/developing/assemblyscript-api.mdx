...
`dataSources` içindeki `context` bölümü, subgraph eşleştirmeleriniz içinde erişilebilen anahtar-değer çiftlerini tanımlamanıza olanak tanır. Kullanılabilir türler `Bool`, `String`, `Int`, `Int8`, `BigDecimal`, `Bytes`, `List` ve `BigInt`'tir.

İşte `context` bölümünde çeşitli tiplerin kullanımını gösteren bir YAML örneği:
...
- `Bool`: Bir Boole değeri belirtir (`true` yada `false`).
- `String`: Bir String(dizgi) değeri belirtir.
- `Int`: 32 bitlik bir tamsayı belirtir.
- `Int`: 8 bitlik bir tamsayı belirtir.
- `BigDecimal`: Ondalık bir sayı belirtir. Tırnak içine alınmalıdır.
- `Bytes`: Onaltılık bir dizgi belirtir.
- `List`: Bir öğeler listesi belirtir. Her öğenin tipini ve verilerini belirtmesi gerekir.
