...
Bu ayrıntılı kılavuz, başarılı bir şekilde subgraph oluşturmak için talimatlar sunmaktadır.

Bir subgraph, bir blokzincirinden veri çıkarır; bu veriyi işler ve GraphQL ile verimli bir şekilde sorgulanabilmesi için veriyi depolar.

![Subgraph Tanımlama](/img/defining-a-subgraph.png)

> Subgraph'inizi The Graph’in merkeziyetsiz ağında kullanabilmek için [Subgraph Studio](https://thegraph.com/studio/apikeys/)’da bir [API anahtarı oluşturmanız](/deploying/subgraph-studio-faqs/#2-how-do-i-create-an-api-key) gerekecek. En az 3.000 GRT ile subgraph’inize sinyal eklemeniz, 2-3 Endeksleyiciyi çekmek için tavsiye edilir.

## Buradan Başlayın

### Graph CLI'ı Yükleyin

Subgraph oluşturmak ve dağıtmak için [Graph CLI](https://github.com/graphprotocol/graph-tooling/tree/main/packages/cli)’ye ihtiyacınız olacak.

Graph CLI, TypeScript ile yazılmıştır ve kullanmak için `node`, ve `npm` veya `yarn` programlarından biri yüklü olmalıdır. En güncel CLI sürümünü [buradan](https://github.com/graphprotocol/graph-tooling/releases?q=%40graphprotocol%2Fgraph-cli&expanded=true) kontrol edin.
...
- Yeni bir subgraph projesi oluşturmak için `graph init` komutu kullanılabilir. Bu komut var olan bir sözleşmeden veya örnek bir subgraph'ten proje başlatabilir.

- Bu `graph init` komutu `--product subgraph-studio` argümanı kullanılarak Subgraph Studio'da bir subgraph da oluşturabilir.

- Tercih ettiğiniz ağda bir akıllı sözleşme dağıtılmışsa, bu sözleşmeyi kullanarak yeni bir subgraph oluşturabilirsiniz.
...
- `<SUBGRAPH_SLUG>` değeri, [Subgraph Studio](https://thegraph.com/studio/)’daki subgraph'inizin ID'sidir. Bu değer subgraph ayrıntıları sayfanızda bulunur.

### Örnek bir subgraph kullanarak
...
- [Örnek subgraph](https://github.com/graphprotocol/example-subgraph), kullanıcı avatarlarını yöneten ve avatarlar oluşturulduğunda veya güncellendiğinde `NewGravatar` veya `UpdateGravatar` olaylarını yayımlayan Dani Grant’in Gravity sözleşmesine dayanır.

- Subgraph, bu olayları `Gravatar` varlıklarını Graph Düğümü deposuna yazarak işler ve bu varlıkların olaylara göre güncellenmesini sağlar.

## Mevcut bir subgraph’e yeni `dataSource` ekleme

`v0.31.0` itibarıyla Graph CLI, `graph add` komutuyla mevcut bir subgraph’e yeni `dataSource` eklemeyi desteklemektedir:
...
`graph add` komutu, ABI’yi Etherscan’den getirir (`--abi` seçeneği ile belirtilen bir ABI yolu tanımlanmadığı müddetçe) ve yeni bir `dataSource` oluşturur. Bu süreç `graph init` komutunun `--from-contract` argümanını kullanarak `dataSource` oluşturmasına ve şema ve mapping'leri buna göre güncellemesine benzerdir. Bu, asıl uygulama sözleşmelerini proxy sözleşmeleri üzerinden endekslemenize olanak tanır.
...
- `--merge-entities` seçeneği, geliştiricinin `entity` ve `event` ad çakışmalarını nasıl ele alacağını belirler:
  ...
  - Eğer `false` ise: yeni bir `entity` (varlık) ve `event` (olay) işleyicileri, `${dataSourceName}{EventName}` şeklinde oluşturulmalıdır.

- Sözleşme `adresi`, ilgili ağ için `networks.json`'a yazılacaktır.

> Not: Etkileşimli CLI kullanıldığında, `graph init` komutunu başarıyla çalıştırdıktan sonra yeni bir `dataSource` eklemeniz istenir.

## Bir subgraph'in bileşenleri

### Subgraph Manifestosu

Subgraph manifestosu, `subgraph.yaml`, subgraph'inizin endeksleyeceği akıllı sözleşmeleri ve ağı, bu sözleşmelerden izlenecek olayları, ve olay verilerinin Graph Düğümü’nün depolayıp sorgulayabileceği varlıklara nasıl dönüştüreceğini tanımlar.

**Subgraph tanımı** şu dosyalardan oluşur:

- `subgraph.yaml`: Subgraph manifestosunu içerir

- `schema.graphql`: Subgraph’inizde depolanan verileri tanımlayan ve GraphQL ile nasıl sorgulanacağını gösteren bir GraphQL şeması

- `mapping.ts`: Şemanızda tanımlanan varlıklara dönüştüren [AssemblyScript Mapping](https://github.com/AssemblyScript/assemblyscript) kodu (örneğin, bu kılavuzdaki `mapping.ts`)
...
- Endekslenmesi gereken her sözleşme için `dataSources` dizisine bir girdi ekler.

Subgraph manifestolarının tam spesifikasyonunu [burada](https://github.com/graphprotocol/graph-node/blob/master/docs/subgraph-manifest.md) bulabilirsiniz.

Yukarıda listelenen örnek subgraph için, `subgraph.yaml` şu şekildedir:
...
- `specVersion`: Subgraph için desteklenen manifest yapısını ve işlevselliği tanımlayan bir semver sürüm numarasıdır. En son sürüm `1.2.0`'dır. Özellikler ve sürümler hakkında daha fazla bilgi için [specVersion sürümlerine](#specversion-releases) bakın.

- `description`: Subgraph’in ne olduğunu insan dilinde bir açıklama ile belirtir. Bu açıklama, subgraph Subgraph Studio'ya dağıtıldığında Graph Gezgini'nde görüntülenir.

- `repository`: Subgraph manifestosunun bulunduğu depo URL'sidir. Bu da Graph Gezgini'nde görüntülenir.

- `features`: kullanılan tüm [özellik(feature)](#experimental-features) adlarının bir listesi.

- `indexerHints.prune`: Subgraph için geçmiş blok verisinin saklama süresini tanımlar. Ayrıntılar için [indexerHints](#indexer-hints) kısmının [budama](#prune) bölümüne bakın.
...
- `dataSources.source.endBlock`: Veri kaynağının endekslemeyi durdurduğu blok numarası (isteğe bağlı). Minimum spec sürüm gereksinimi: `0.0.9`.
...
> **Not**: Yeni [dinamik veri kaynakları](#data-source-templates-for-dynamically-created-contracts) oluşturulduğunda, bu kaynaklar için tanımlanan işleyiciler, yalnızca mevcut veri kaynağı işleyicileri tamamen işlendiğinde çalışmaya başlar ve tetiklendiğinde aynı sırada tekrar eder.

### Endekslenmiş Argüman Filtreleri / Konu Filtreleri

> [SpecVersion](#specversion-releases) >= `1.2.0` **gerektirir**
...
- Olayın ilk endekslenmiş argümanı `topic1` ile, ikinci argümanı `topic2` ile ve üçüncü argümanı `topic3` ile eşleşir; çünkü Ethereum Sanal Makinesi (EVM) her olay için en fazla üç endekslenmiş argümana izin verir.
...
- `Transfer` olayı, adresler arasında yapılan token işlemlerini kaydetmek için kullanılır.
- `from` ve `to` parametreleri endekslenmiştir. Bu da olay dinleyicilerinin belirli adresleri içeren transferleri filtrelemesine ve izlemesine olanak tanır.
- `transfer` fonksiyonu, bir token transferi işlemini basit bir şekilde temsil eder ve her çağrıldığında Transfer olayını yayar.
...
- `topic1` olayın ilk endekslenmiş argümanına, `topic2` ikinci argümana ve `topic3` üçüncü argümana karşılık gelir.
...
- `topic1`, `0xAddressA`'nin gönderici olduğu `Transfer` olaylarını filtreleyecek şekilde yapılandırılmıştır.
- `topic2`, `0xAddressB`'nin alıcı olduğu `Transfer` olaylarını filtreleyecek şekilde yapılandırılmıştır.
- Subgraph yalnızca `0xAddressA`'dan `0xAddressB`'ye doğrudan gerçekleşen işlemleri endeksleyecektir.
...
- `topic1`, göndericinin `0xAddressA`, `0xAddressB` veya `0xAddressC` olduğu `Transfer` olaylarını filtreleyecek şekilde yapılandırılmıştır.
- `topic2`, alıcının `0xAddressB` veya `0xAddressC` olduğu `Transfer` olaylarını filtreleyecek şekilde yapılandırılmıştır.
...
Bildirimsel `eth_call`, `eth_call`'ın önceden gerçekleştirilmesini sağlayan ve `graph-node`'un bunları paralel olarak çalıştırmasına olanak tanıyan önemli bir subgraph özelliğidir.
...
- Bildirimsel `eth_calls`: Ardışık değil, paralel olarak yürütülmek üzere tanımlanmış Ethereum çağrıları.
...
#### Bildirimsel `eth_calls` Kullanılmayan Senaryo
...
Bildirilmiş `eth_calls`, altta yatan olayın `event.address` bilgisine ve tüm `event.params` değerlerine erişebilir.

`event.address` kullanan `Subgraph.yaml`:
...
- `global0X128`, bildirilmiş `eth_call` çağrısıdır.
- (`global0X128`) metni, hataları kaydederken kullanılan bu `eth_call` için etikettir.
- (`Pool[event.address].feeGrowthGlobal0X128()`) metni, çalıştırılacak olan gerçek `eth_call` çağrısıdır ve `Contract[address].function(arguments)` biçimindedir.
- `address` ve `arguments`, işleyici çalıştırıldığında kullanılabilir olacak değişkenlerle değiştirilebilir.

`event.params` kullanan `Subgraph.yaml`:
...
| Sürüm | Sürüm Notları                                                                                                                                                          |
|:-----:| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.2.0 | [Endekslenmiş Argüman Filtreleme](/#indexed-argument-filters--topic-filters) ve bildirilmiş `eth_call` desteği eklendi.                                                |
| 1.1.0 | [Zaman Serileri & Birleştirmeler](#timeseries-and-aggregations) desteği eklendi. `id` için `Int8` türü desteği eklendi.                                                |
| 1.0.0 | Subgraph'leri budamak için [`indexerHints`](/developing/creating-a-subgraph/#indexer-hints) özelliği desteği                                                           |
| 0.0.9 | `endBlock` özelliği desteği                                                                                                                                            |
| 0.0.8 | [Blok İşleyicileri](/developing/creating-a-subgraph/#polling-filter) ve [İlklendirme İşleyicileri](/developing/creating-a-subgraph/#once-filter) için yoklama desteği. |
| 0.0.7 | [Dosya Veri Kaynakları](/developing/creating-a-subgraph/#file-data-sources) desteği eklendi.                                                                           |
| 0.0.6 | Hızlı [Endeksleme Kanıtı](/network/indexing/#what-is-a-proof-of-indexing-poi) hesaplama varyantı desteği.                                                              |
...
- Herkese açık bir proje için subgraph oluşturuyorsanız, projeyi bilgisayarınıza indirip [`npx hardhat compile`](https://hardhat.org/hardhat-runner/docs/guides/compile-contracts#compiling-your-contracts) komutu veya `solc` ile derleyerek ABI’yi elde edebilirsiniz.
...
Subgraph’iniz için şema, `schema.graphql` dosyasındadır. GraphQL şemaları, GraphQL arayüz tanımlama dili kullanılarak tanımlanır. Daha önce hiç GraphQL şeması yazmadıysanız, GraphQL tür sistemi hakkında bilgi veren bu başlangıç kılavuzuna göz atmanız önerilir. GraphQL şemaları için referans dokümantasyonuna [GraphQL API](/querying/graphql-api/) bölümünden ulaşabilirsiniz.
...
- Tüm sorgular, subgraph şemasında tanımlanan veri modeline ve subgraph tarafından endekslenen varlıklara göre yapılacaktır. Bu nedenle, subgraph şemasını dapp'inizin ihtiyaçlarına uygun bir şekilde tanımlamak iyi bir yaklaşımdır.
- Varlıkları, olaylar veya fonksiyonlar yerine “veri içeren nesneler” olarak düşünmek faydalı olabilir.
- Varlık türlerini `schema.graphql` dosyasında tanımlarsınız ve Graph Düğümü, bu varlık türünün tekil örnekleri ve koleksiyonları için sorgulanabilir üst seviye alanlar oluşturur.
- Her varlık olması gereken türün, `@entity` direktifiyle belirtilmesi zorunludur.
- Varsayılan olarak, varlıklar değiştirilebilirdir; yani, eşlemeler halihazırda mevcut varlıkları yükleyebilir, onları modifiye edebilir ve o varlığın yeni bir sürümünü saklayabilir.
  - Değiştirilebilirlik maliyeti beraberinde getirir. Bu nedenle zincirden alınan verileri içerenler gibi asla değiştirilmeyecek varlık türleri için, `@entity(immutable: true)` ile değişmez olarak işaretlenmeleri önerilir.
...
Aşağıdaki `GravatarAccepted` ve `GravatarDeclined` örnek varlıkları olaylara dayanmaktadır. Olayları veya işlev çağrılarını bire bir varlıklarla eşlemek önerilmez.
...
| `Int`           | GraphQL spesifikasyonu, `Int`'i işaretli (signed) 32-bit bir tamsayı olarak tanımlar.                                                                                                                                                |
| `Int8`          | 8 baytlık işaretli bir tamsayı, diğer adıyla 64-bit işaretli tamsayı -9.223.372.036.854.775.808 ile 9.223.372.036.854.775.807 aralığında değerler saklayabilir. Ethereum'daki `i64`'ü temsil etmek için bunu kullanmayı tercih edin. |
...
| `Timestamp`     | Timestamp, mikrosaniye cinsinden bir `i64` değeridir. Genellikle `timestamp` alanları için, zaman serileri ve toplulaştırmalar amacıyla kullanılır.                                                                                  |
...
GraphQL spesifikasyonuna göre şema varlık özniteliklerinin üzerine `#` sembolü kullanılarak yorum eklenebilir. Bu, aşağıdaki örnekte gösterilmiştir:
...
Eşlemeler, belirli bir kaynaktan veriyi alır ve bunu şemanızda tanımlanan varlıklara dönüştürür. Eşlemeler, [WASM](https://webassembly.org/) (WebAssembly) olarak derlenebilen, [AssemblyScript](https://github.com/AssemblyScript/assemblyscript/wiki) adı verilen bir [TypeScript](https://www.typescriptlang.org/docs/handbook/typescript-in-5-minutes.html) alt kümesi programla diliyle yazılır. AssemblyScript normal TypeScript'ten daha katıdır ancak tanıdık bir sentaks sunar.
...
`id` alanları için `Bytes` türünü kullanmanız şiddetle tavsiye edilir. Yalnızca gerçekten insan tarafından okunabilir metinler içeren öznitelikler (örneğin, bir token'ın adı gibi) için `String` kullanmalısınız. Aşağıda, yeni varlıklar oluştururken dikkate almanız gereken bazı önerilen `id` değerleri verilmiştir.

- `transfer.id = event.transaction.hash`

- `let id = event.transaction.hash.concatI32(event.logIndex.toI32())`

- Toplulaştırılmış verileri (örneğin günlük ticaret hacimleri) depolayan varlıklar için, `id` genellikle gün numarasını içerir. Bu örnekte `id`'yi `Bytes` kullanarak tanımlamak faydalıdır. `id` değerini şöyle belirleyebilirsiniz:
...
- Sabit adresleri `Bytes`'a dönüştürün.

`const id = Bytes.fromHexString('0xdead...beef')`

[Graph Typescript Kütüphanesi](https://github.com/graphprotocol/graph-tooling/tree/main/packages/ts), Graph Düğümü deposuyla etkileşim kurmaya yönelik yardımcı uygulamalar ve akıllı sözleşme verileri ile varlıklarını işlemek için kolaylık sağlayan araçlar içerir. Bu kütüphane, `@graphprotocol/graph-ts` üzerinden `mapping.ts` dosyasına aktarılabilir.
...
Kod üretimi, `src/mapping.ts` dosyasındaki eşleme kodunu kontrol etmez. Subgraph'inizi Graph Gezgini'ne dağıtmadan önce bu kodu kontrol etmek isterseniz, `yarn build` komutunu çalıştırabilir ve TypeScript derleyicisinin bulabileceği sentaks hatalarını düzeltebilirsiniz.
...
Bu alt sözleşmelerin adresleri önceden bilinebilir ya da bilinmeyebilir ve bu sözleşmelerin birçoğu zamanla oluşturulabilir ve/veya eklenebilir. Bu nedenle, böyle durumlarda tek bir veri kaynağı veya sabit sayıda veri kaynağı tanımlamak imkansızdır ve daha dinamik bir yaklaşım gereklidir. Bu yaklaşım: _veri kaynağı şablonları_.
...
`indexerHints` ayarı, bir subgraph'in manifestosunda, endeksleyicilere bir subgraph'i işleme ve yönetme konusunda yönergeler sağlar. Bu ayar, veri işleme, endeksleme stratejileri ve optimizasyonlar dahil olmak üzere operasyonel kararları etkiler. Şu anda, geçmiş veri saklama veya budama yönetimi için `prune` seçeneğini içerir.

> Bu özellik, `specVersion: 1.0.0`'dan itibaren kullanılabilir

### Budama

`indexerHints.prune`: Bir subgraph için geçmiş blok verilerinin saklama süresini tanımlar. Şu seçenekleri kabul eder:

1.  `"never"`: Tarihsel veriler budanmaz, tüm geçmişi saklar.
2.  `"auto"`: Endeksleyici tarafından atanmış minimum miktarda geçmiş veriyi tutar, sorgu performansını optimize eder.
...
- [Zaman yolculuğu sorguları](/querying/graphql-api/#time-travel-queries): varlıkların subgraph'in geçmişinde belirli bloklardaki eski durumlarını sorgulamayı sağlar
- Verilen subgraph'i bulunduğu blokta başka bir subgraph için [nakil tabanı](/developing/creating-a-subgraph/#grafting-onto-existing-subgraphs) olarak kullanmak
...
> Genellikle `"auto"` seçeneğinin kullanılması tavsiye edilir. Bu seçenek sorgu performansını maksimize eder ve kapsamlı tarihsel veri erişimi gerektirmeyen çoğu kullanıcı için yeterlidir.

[Zaman yolculuğu sorgularını](/querying/graphql-api/#time-travel-queries) kullanan subgraph'lerde, tarihsel veri saklama için belirli bir blok sayısı ayarlamanız veya tüm tarihsel varlık durumlarını korumak için `prune: never` ayarını kullanmanız önerilir. Aşağıda subgraph ayarlarınızda her iki seçeneği de nasıl yapılandıracağınıza dair örnekler verilmiştir:

Belirli bir miktarda tarihsel veri saklamak için:

```
 indexerHints:
  prune: 1000 # 1000 sayısını saklamak istediğiniz blok sayısıyla değiştirin
```
...
Zaman serisi varlıkları, `@entity(timeseries: true)` ile schema.graphql dosyasında tanımlanır. Her zaman serisi varlığının int8 türünde benzersiz bir kimliği, Timestamp türünde bir zaman damgası olmalı ve zaman serisi varlıklar toplulaştırma varlıklarının hesaplamalarında kullanılacak verileri içermelidir. Bu Zaman Serisi varlıklar, düzenli tetikleyici işleyicilerde saklanabilir ve Toplulaştırma varlıkları için “ham veri” görevi görür.

Toplulaştırma varlıkları, schema.graphql dosyasında `@aggregation` ile tanımlanır. Her bir toplulaştırma varlığı, veriyi toplayacağı kaynağı (Zaman Serisi varlığı olmalıdır) ve kullanacağı aralıkları (örneğin, saat, gün) belirler ve kullanacağı toplulaştırma fonksiyonunu (örneğin, toplam, sayma, min, maks, ilk, son) tanımlar. Toplulaştırma varlıkları, belirtilen kaynak temelinde gerekli aralığın sonunda otomatik olarak hesaplanır.

#### Mevcut Toplulaştırma Aralıkları

- `hour`: Zaman serisi periyodunu her saat saat başına ayarlar.
- `day`: Zaman serisi periyodunu her gün olarak ayarlar, 00:00’da başlar ve biter.
...
- `min`: Minimum değer.
- `max`: Maksimum değer.
- `first`: Periyottaki ilk değer.
- `last`: Periyottaki son değer.
...
Zaman Serisi ve Toplulaştırmalar hakkında daha fazla bilgi için [burayı okuyun](https://github.com/graphprotocol/graph-node/blob/master/docs/aggregations.md).
...
Bu işlem, zincir tabanlı veri kaynaklarını dinamik olarak oluşturmak için kullanılan [mevcut veri kaynağı şablonlarına](/developing/creating-a-subgraph/#data-source-templates) benzer.
...
> İç içe varlıklar temelinde üst varlıkları filtrelemek için [iç içe filtreler](/querying/graphql-api/#example-for-nested-entity-filtering) kullanılabilir.
...
Dosya veri kaynağı, etkileşimde bulunacağı tüm varlık türlerini `entities` altında açıkça belirtmelidir. Daha fazla ayrıntı için [kısıtlamalar](#limitations) bölümüne bakın.

#### Dosyaları işlemek için yeni bir işleyici oluşturun

Bu işleyici, dosyanın içeriği olan bir `Bytes` parametresini kabul etmelidir. böylece bu içerik daha sonra işlenebilir. Bu genellikle, `graph-ts` yardımcıları ile işlenebilen ([dökümantasyon burada](/developing/graph-ts/api/#json-api)) bir JSON dosyası olacaktır.
...
Arweave için, Graph Düğümü 0.33.0 sürümü itibariyle Arweave ağ geçidinden ([örnek dosya](https://bdxujjl5ev5eerd5ouhhs6o4kjrs4g6hqstzlci5pf6vhxezkgaa.arweave.net/CO9EpX0lekJEfXUOeXncUmMuG8eEp5WJHXl9U9yZUYA)) [işlem ID](https://docs.arweave.org/developers/arweave-node-server/http-api#transactions)'si ile saklanan dosyaları getirebilir. Arweave, Irys (eski adıyla Bundlr) üzerinden yüklenen işlemleri destekler ve Graph Düğümü, dosyaları [Irys manifestolarına](https://docs.irys.xyz/overview/gateways#indexing) göre de getirebilir.
...
Dosya Veri Kaynakları oluştururken ekstra bilgileri işleyiciye iletmek için [DataSource context](/developing/graph-ts/api/#entity-and-datasourcecontext) kullanılabilir. Bu bilgiler Dosya Veri Kaynağı işleyicisi tarafından kullanılabilecektir.
...
Dosya Veri Kaynakları için işleyiciler, `eth_call` sözleşme bağlamalarını içeren dosyalarda bulunamaz; aksi takdirde, “unknown import: `ethereum::ethereum.call` has not been defined” hatası verir ([Github issue'su](https://github.com/graphprotocol/graph-node/issues/4309)). Çözüm olarak, dosya veri kaynağı işleyicilerini ayrı bir dosyada oluşturabilirsiniz.
