...
Индексаторы — это операторы узлов в The Graph Network, которые размещают токены Graph (GRT) для предоставления услуг индексирования и обработки запросов. Индексаторы получают оплату за запросы и вознаграждение за свои услуги индексирования. Они также зарабатывают из Пула Скидок, который распределяется между всеми участниками сети пропорционально их работе в соответствии с функцией скидок Кобба-Дугласа.
...
| Настройка | Postgres<br />(ЦП) | Postgres<br />(память в ГБ) | Postgres<br />(диск в ТБ) | ВМ<br />(ЦП) | ВМ<br />(память в ГБ) |
| --- | :-: | :-: | :-: | :-: | :-: |
| Небольшой | 4 | 8 | 1 | 4 | 16 |
| Стандартный | 8 | 30 | 1 | 12 | 48 |
| Средний | 16 | 64 | 2 | 32 | 64 |
| Большой | 72 | 468 | 3.5 | 48 | 184 |
...
| Порт | Назначение | Маршруты | CLI-аргумент | Переменная среды |
| --- | --- | --- | --- | --- |
| 8000 | HTTP-сервер GraphQL<br />(для запросов субграфов) | /subgraphs/id/...<br />/subgraphs/name/.../... | --http-port | - |
| 8001 | GraphQL WS<br />(для подписок на субграфы) | /subgraphs/id/...<br />/subgraphs/name/.../... | --ws-port | - |
| 8020 | JSON-RPC<br />(для управления развертываниями) | / | --admin-port | - |
| 8030 | API статуса индексации субграфа | /graphql | --index-node-port | - |
...
| Порт | Назначение | Маршруты | CLI-аргумент | Переменная среды |
| --- | --- | --- | --- | --- |
| 7600 | HTTP-сервер GraphQL<br />(для платных запросов субграфов) | /subgraphs/id/...<br />/status<br />/channel-messages-inbox | --port | `INDEXER_SERVICE_PORT` |
...
### Настройте инфраструктуру сервера с помощью Terraform в Google Cloud

> Примечание: Индексаторы в качестве альтернативы и могут использовать AWS, Microsoft Azure или Alibaba.
...
#### Интерфейс командной строки индексатора
...
- Индексатор (или другое программное обеспечение) может утверждать или отменять действия в очереди, используя `indexer-cli`. Команды утверждения и отмены принимают в качестве входных данных массив идентификаторов действий.
...
- `Allocate` — выделить ставку для развертывания определённого субграфа
