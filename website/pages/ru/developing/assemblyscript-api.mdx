...
Объекты, записанные в хранилище, сопоставляются один к одному с типами `@entity`, определенными в схеме субграфов GraphQL. Чтобы сделать работу с этими объектами удобной, команда `graph codegen`, предоставляемая [Graph CLI](https://github.com/graphprotocol/graph-cli) генерирует классы объектов, которые являются подклассами встроенного типа `Entity`, с методами получения и настройки свойств полей в схеме, а также методами загрузки и сохранения этих объектов.
