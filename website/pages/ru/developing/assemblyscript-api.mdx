...
```typescript
// Импортируйте класс событий Transfer, сгенерированный из ERC20 ABI
import { Transfer as TransferEvent } from '../generated/ERC20/ERC20'

// Импортируйте тип объекта Transfer, сгенерированный из схемы GraphQL 
import { Transfer } from '../generated/schema'
событие
// Обработчик события передачи
экспортирует функцию handleTransfer(event: TransferEvent): void {
  // Создайте объект Transfer, используя хеш транзакции в качестве идентификатора объекта
let id = event.transaction.hash
  let transfer = new Transfer(id)

  // Установите свойства объекта, используя параметры события
transfer.from = event.params.from
  transfer.to = event.params.to
  transfer.amount = event.params.amount

  // Сохраните объект в хранилище
transfer.save()
}
```
