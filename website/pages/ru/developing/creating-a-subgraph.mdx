...
Следующее подробное руководство содержит инструкции по успешному созданию субграфа.

Субграф извлекает данные из блокчейна, обрабатывает их и сохраняет для эффективного выполнения запросов через GraphQL.
...
Для создания и развертывания субграфа Вам понадобится [Graph CLI](https://github.com/graphprotocol/graph-tooling/tree/main/packages/cli).

Интерфейс командной строки The Graph написан на TypeScript, и для его использования необходимо установить `node` и `npm` или `yarn`. Проверьте наличие [самой последней](https://github.com/graphprotocol/graph-tooling/releases?q=%40graphprotocol%2Fgraph-cli&expanded=true) версии CLI.
...
- Команду `graph init` можно использовать для настройки нового проекта субграфа либо на основе существующего контракта, либо на основе примера субграфа.

- Эта команда `graph init` также позволяет создать субграф в Subgraph Studio, передав `--product subgraph-studio`.

- Если у Вас уже есть смарт-контракт, развернутый в выбранной Вами сети, Вы можете загрузить новый субграф из этого контракта, чтобы начать работу.
...
- `<SUBGRAPH_SLUG>` - это идентификатор Вашего субграфа в [Subgraph Studio](https://thegraph.com/studio/). Его можно найти на странице сведений о субграфе.
...
- [Пример субграфа](https://github.com/graphprotocol/example-subgraph) основан на контракте Gravity, разработанном Дэни Грантом, который управляет аватарами пользователей и генерирует события `NewGravatar` или `UpdateGravatar` каждый раз, когда аватары создаются или обновляются.

- Субграф обрабатывает эти события, записывая объекты `Gravatar` в хранилище Graph Node и обеспечивая их обновление в соответствии с событиями.

## Добавление новых `dataSources` к существующему субграфу

Начиная с версии `v0.31.0`, Graph CLI поддерживает добавление новых `dataSources` к существующему субграфу с помощью команды `graph add`:
...
Команда `graph add` извлечет ABI из Etherscan (если путь ABI не указан с помощью опции `--abi`) и создаст новый `dataSource`, аналогично тому, как команда `graph init` создает `dataSource` `--from-contract`, соответствующим образом обновляя схему и мэппинги. Это позволяет индексировать контракты реализации на основе их прокси-контрактов.
...
- Параметр `--merge-entities` определяет, как разработчик хотел бы обрабатывать конфликты имен `entity` и `event`:
  ...
  - Если `false`: необходимо создать новый обработчик `entity` & `event` с помощью `${dataSourceName}{EventName}`.
...
Манифест субграфа, `subgraph.yaml`, определяет смарт-контракты & сеть, которую Ваш субграф будет индексировать, события из этих контрактов, на которые следует обращать внимание, а также то, как сопоставить данные событий с объектами, которые Graph Node хранит и делает доступными для запросов.

**Определение субграфа** состоит из следующих файлов:

- `subgraph.yaml`: Содержит манифест субграфа

- `schema.graphql`: Схема GraphQL, определяющая данные, хранящиеся для Вашего субграфа, и способы запроса к ним через GraphQL

- `mapping.ts`: Код [AssemblyScript Mappings](https://github.com/AssemblyScript/assemblyscript), который преобразует данные событий в объекты, определенные в Вашей схеме (например, `mapping.ts` в этом руководстве)
...
- Добавлять запись для каждого контракта, требующего индексирования, в массив `dataSources`.

Полную спецификацию манифестов субграфов можно найти [здесь](https://github.com/graphprotocol/graph-node/blob/master/docs/subgraph-manifest.md).

Для приведенного выше примера субграфа, `subgraph.yaml` выглядит следующим образом:
...
- `specVersion`: версия semver, которая определяет поддерживаемую структуру и функциональность манифеста для субграфа. Последняя версия — `1.2.0`. См. раздел [выпуски specVersion](#specversion-releases), чтобы получить более подробную информацию о функциях & выпусках.
...
> **Требуется**: [SpecVersion](#specversion-releases) >= `1.2.0`
...
- Первый индексированный аргумент события соответствует `topic1`, второй — `topic2` и т. д. до `topic3`, поскольку виртуальная машина Ethereum (EVM) допускает до трех индексированных аргументов на одно событие.
...
- Параметры `from` и `to` индексируются, что позволяет слушателям событий фильтровать и отслеживать переводы, включающие определенные адреса.
...
- `topic1` настроен на фильтрацию событий `Transfer`, где `0xAddressA`, `0xAddressB`, `0xAddressC` является отправителем.
- `topic2` настроен на фильтрацию событий `Transfer`, где `0xAddressB` и `0xAddressC` является получателем.
...
Декларативные `eth_calls` — это полезная функция субграфов, которая позволяет выполнять `eth_calls` досрочно, что дает возможность `graph-node` выполнять их параллельно.
...
#### Сценарий с декларативными `eth_calls`
...
Декларированный `eth_calls` может получить доступ к `event.address` базового события, а также ко всем `event.params`.

`Subgraph.yaml` с использованием `event.address`:
...
- `global0X128` — это декларированный `eth_call`.
- Текст (`global0X128`) представляет собой метку для этого кода `eth_call`, который используется при регистрации ошибок.
...
|:------:| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.2.0  | Добавлена ​​поддержка [Индексированной фильтрации аргументов](/#indexed-argument-filters--topic-filters) & декларирован `eth_call`                                                |
| 1.1.0  | Поддерживает [Таймсерии & Агрегации](#timeseries-and-aggregations). Добавлена ​​поддержка типа `Int8` для `id`.                                                                   |
| 1.0.0  | Поддерживает функцию [`indexerHints`](/developing/creating-a-subgraph/#indexer-hints) для сокращения субграфов                                                                    |
| 0.0.9  | Поддерживает функцию `endBlock`                                                                                                                                                   |
| 0.0.8  | Добавлена ​​поддержка опроса [Обработчиков блоков](/developing/creating-a-subgraph/#polling-filter) и [Обработчиков инициализации](/developing/creating-a-subgraph/#once-filter). |
| 0.0.7  | Добавлена поддержка [Источников файловых данных](/developing/creating-a-subgraph/#file-data-sources).                                                                             |
| 0.0.6  | Поддерживает вариант быстрого расчета [Подтверждения индексирования](/network/indexing/#what-is-a-proof-of-indexing-poi).                                                         |
...
- Вы определяете типы объектов в `schema.graphql`, а Graph Node сгенерирует поля верхнего уровня для запроса отдельных экземпляров и коллекций этого типа объектов.
