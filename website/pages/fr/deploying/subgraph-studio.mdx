---
title: Comment utiliser Subgraph Studio
---

Bienvenue dans votre nouveau site de Lancement 👩🏽‍🚀

Subgraph Studio est votre espace pour créer et construire des subgraphs, ajouter des métadonnées et les publier dans le nouvel Explorer décentralisé (plus d'informations [ici](/network/explorer)).
...
Ici, dans Subgraph Studio, vous avez un contrôle total sur vos subgraphs. Non seulement vous pouvez tester vos subgraphs avant de les publier, mais vous pouvez également restreindre vos clés API à des domaines spécifiques et autoriser uniquement certains Indexeurs à interroger à partir de leurs clés API.
...
1. Connectez-vous avec votre portefeuille - vous pouvez le faire via MetaMask, WalletConnect, Coinbase Wallet ou Safe.
...
Après avoir créé votre subgraph, vous pourrez le déployer en utilisant la [CLI](https://github.com/graphprotocol/graph-tooling/tree/main/packages/cli), ou interface en ligne de commande. Le déploiement d'un subgraph avec la CLI le poussera vers le Studio où vous pourrez tester les subgraphs en utilisant l'environnement de test (playground). Cela vous permettra éventuellement de publier sur The Graph Network. Pour plus d'informations sur la configuration de la CLI, [consultez ceci](/developing/defining-a-subgraph#install-the-graph-cli) (psst, assurez-vous d'avoir votre clé de déploiement à portée de main). N'oubliez pas que déployer n'est **pas la même chose que** publier. Lorsque vous déployez un subgraph, vous le poussez simplement vers le Studio où vous pouvez le tester. En revanche, lorsque vous publiez un subgraph, vous le publiez sur la chaîne.
...
Pour publier votre subgraph avec succès, vous devrez suivre les étapes décrites dans cette [section](/publishing/publishing-a-subgraph/).

Jetez un œil à l'aperçu vidéo ci-dessous également :

<VideoEmbed youtube="satdwdbvzAQ" />

N'oubliez pas qu'au cours de votre processus de publication, vous pourrez pousser soit vers Arbitrum One ou Arbitrum Sepolia. Si vous êtes un développeur subgraph débutant, nous vous suggérons fortement de commencer par publier sur Arbitrum Sepolia, ce qui est gratuit. Cela vous permettra de voir comment le subgraph fonctionnera dans Graph Explorer et de tester les éléments de curation.
...
Les développeurs peuvent vouloir mettre à jour leur subgraph pour diverses raisons. Dans ce cas, vous pouvez déployer une nouvelle version de votre subgraph dans le Studio en utilisant la CLI (cette version sera uniquement privée à ce moment-là) et si vous en êtes satisfait, vous pouvez publier ce nouveau déploiement dans Graph Explorer. Cela créera une nouvelle version de votre subgraph sur laquelle les Curateurs pourront commencer à signaler et les Indexeurs pourront indexer cette nouvelle version.

Jusqu'à récemment, les développeurs étaient obligés de déployer et de publier une nouvelle version de leur subgraph dans l'Explorer pour mettre à jour les métadonnées de leurs subgraphs. Désormais, les développeurs peuvent mettre à jour les métadonnées de leurs subgraphs **sans avoir à publier une nouvelle version**. Ils peuvent mettre à jour les détails de leur subgraph dans le Studio (sous la photo de profil, le nom, la description, etc.) en cochant une option appelée **Mettre à jour les détails** dans Graph Explorer. Si cette option est cochée, une transaction on-chain sera générée pour mettre à jour les détails du subgraph dans l'Explorer sans avoir à publier une nouvelle version avec un nouveau déploiement.
