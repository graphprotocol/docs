...
Une fois défini, le subgraph peut être construit et déployé dans [Subgraph Studio](https://thegraph.com/docs/en/deploying/subgraph-studio-faqs/). Subgraph Studio est un environnement sandbox qui indexera le subgraph déployé et le rendra disponible pour un développement et des tests à débit limité. Cela donne aux développeurs la possibilité de vérifier que leur subgraph ne rencontre aucune erreur d'indexation et fonctionne comme prévu.
