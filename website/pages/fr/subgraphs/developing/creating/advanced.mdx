...
Les entités de séries chronologiques sont définies avec `@entity(timeseries: true)` dans schema.graphql. Chaque entité de série chronologique doit avoir un ID unique de type int8, une timestamp de type Timestamp et inclure des données qui seront utilisées pour le calcul par des entités d'agrégation. Ces entités de séries chronologique peuvent être enregistrées dans des gestionnaires triggers habituels et agissent comme les "données brutes" pour les entités d'agrégation.

Les entités d'agrégation sont définies avec `@aggregation` dans schema.graphql. Chaque entité d'agrégation définit la source à partir de laquelle elle collectera des données (qui doit être une entité de série chronologique ), définit les intervalles (par exemple, heure, jour) et spécifie la fonction d'agrégation qu'elle utilisera (par exemple, somme, compte, min, max, premier, dernier). Les entités d'agrégation sont automatiquement calculées sur la base de la source spécifiée à la fin de l'intervalle requis.
