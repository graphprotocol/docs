...
Ce livre de recettes nécessite [yarn](https://yarnpkg.com/), [les dépendances nécessaires pour le développement local de Substreams](https://substreams.streamingfast.io/documentation/consume/installing-the-cli), et la dernière version de Graph CLI (>=0.52.0) :
...
modules: # spécifiez les modules avec leurs entrées et sorties.
  - name: map_contract
    kind: map
    inputs:
...
> Currently, Subgraph Studio and The Graph Network support Substreams-powered subgraphs. Review [supported networks](/supported-networks/) for additional details.
