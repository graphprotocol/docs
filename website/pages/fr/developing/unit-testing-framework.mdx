...
```sh
préparer l'installation de postgresql
```
...
```
<PROJECT_PATH>/node_modules/gluegun/build/index.js:13 lancer ;
```
...
```json
{
   "name": "subgraph de démonstration",
   "version": "0.1.0",
   "scripts": {
     "test": "test graphique",
     ...
...
```sh
  erreur de l'expéditeur : failed to xattr node_modules/binary-install-raw/bin/binary-<platform> : permission denied
```
...
`décrivez(name: String , () => {})` - Définit un groupe de test.
...
décrivez ("handleNewGravatar()", () => {
  testez ("Devrait créer une nouvelle entité", () => {
    ...
  })
})
```
...
```typescript
fieldEquals (entityType : chaîne, identifiant : chaîne, fieldName : chaîne, ExpectVal : chaîne)

est égal (attendu : ethereum.Value, réel : ethereum.Value)

notInStore (entityType : chaîne, identifiant : chaîne)

adresseEquals (adresse1 : adresse, adresse2 : adresse)

bytesEquals (octets1 : octets, octets2 : octets)

i32Equals(numéro1 : i32, numéro2 : i32)

bigIntEquals (bigInt1 : BigInt, bigInt2 : BigInt)

booleanEquals(bool1 : booléen, bool2 : booléen)

stringEquals(string1 : chaîne, string2 : chaîne)

arrayEquals (tableau1 : tableau<ethereum.Value>, tableau2 : tableau<ethereum.Value>)

tupleEquals (tuple1 : ethereum.Tuple, tuple2 : ethereum.Tuple)

assertTrue(valeur : booléenne)

assertNull<T>(valeur : T)

assertNotNull<T>(valeur : T)

EntityCount (EntityType : chaîne, ExpectedCount : i32)
```
...
test('Peut appeler des mappages avec des événements personnalisés', () => {
   // Créez une entité de test et enregistrez-la dans le magasin comme état initial (facultatif)
   ...
   // Appelez les fonctions de mappage en passant les événements que nous venons de créer
   handleNewGravatars([newGravatarEvent, anotherGravatarEvent])

   // Affirmer l'état du magasin
   assert.fieldEquals('Gravatar', 'gravatarId0', 'id', 'gravatarId0')
   assert.fieldEquals('Gravatar', '12345', 'propriétaire', '0x89205A3A3b2A69De6Dbf7f01ED13B2108B2c43e7')
   assert.fieldEquals('Gravatar', '3546', 'displayName', 'cap')

   // Vider le magasin afin de démarrer le prochain test sur une table rase
   clearStore()
})
...
// Exportation du callback ipfs.map() pour que matchtck le détecte.
...
```typescript
test('Exemple de test de champs dérivés', () => {
   laissez mainAccount = new GraphAccount('12')
   mainAccount.save()
   laissez exploitéAccount = new GraphAccount ('1')
   OperatorAccount.operators = ['12']
   exploitéAccount.save()
