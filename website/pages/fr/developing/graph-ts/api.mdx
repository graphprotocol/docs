...
- la [bibliothèque Graph TypeScript] (https://github.com/graphprotocol/graph-tooling) (`graph-ts`) et
...
Le solde d'une adresse peut être récupéré à l'aide du module `ethereum`. Cette fonctionnalité est disponible à partir de `apiVersion : 0.0.9` défini `subgraph.yaml`. `getBalance()` récupère le solde de l'adresse spécifiée du bloc actuellement en cours de traitement dans le subgraph.
...
laisser l'équilibre = ethereum.getBalance(address) // returns équilibre dans BigInt
```
...
| Chaîne                                  | BigInt                                  | BigInt.fromString(s)                            |
