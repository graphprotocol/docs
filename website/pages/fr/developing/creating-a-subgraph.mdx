...
Les `eth_calls` déclarés peuvent accéder à l'`event.address` de l'événement sous-jacent ainsi qu'à tous les `event.params`.
...
- `global0X128` est l'`eth_call` déclaré.
- Le texte (`global0X128`) est l'étiquette pour cet `eth_call` utilisé lors de la journalisation des erreurs.
- Le texte (`Pool[event.address].feeGrowthGlobal0X128()`) est l'`eth_call` réel qui sera exécuté, et qui est sous la forme `Contract[address].function(arguments)`
- Les `address` et `arguments` peuvent être remplacés par des variables disponibles lorsque le gestionnaire (handler) est exécuté.
...
|:-------:| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|  1.2.0  | Ajout de la prise en charge du [Filtrage d'Arguments Indexés](/#indexed-argument-filters--topic-filters) & les `eth_call` déclarés                                                                            |
|  1.1.0  | Prise en charge des [Séries chronologiques & Agrégations](#timeseries-and-aggregations). Ajout du support pour le type `Int8` pour `id`.                                                                      |
|  1.0.0  | Supporte la fonctionnalité[`indexerHints`](/developing/creating-a-subgraph/#indexer-hints) pour purger les subgraphs                                                                                          |
|  0.0.9  | Prise en charge de la fonctionnalité `endBlock`                                                                                                                                                               |
|  0.0.8  | Ajout de la prise en charge des sondages des [Gestionnaires de Blocs](/developing/creating-a-subgraph/#polling-filter) et des [Gestionnaires d'Initialisation](/developing/creating-a-subgraph/#once-filter). |
|  0.0.7  | Ajout de la prise en charge pour les [File Data Sources](/developing/creating-a-subgraph/#file-data-sources).                                                                                                 |
|  0.0.6  | Prend en charge une variante rapide du calcul de la [Preuve d'Indexation (Proof of Indexing)](/network/indexing/#what-is-a-proof-of-indexing-poi).                                                            |
...
Le schéma de votre subgraph se trouve dans le fichier `schema.graphql`. Les schémas GraphQL sont définis en utilisant le langage de définition d'interface GraphQL. Si vous n'avez jamais écrit de schéma GraphQL, il est recommandé de consulter cette introduction sur le système de types GraphQL. La documentation de référence pour les schémas GraphQL peut être trouvée dans la section [API GraphQL](/querying/graphql-api/).
...
- Toutes les requêtes seront effectuées à partir du modèle de données défini dans le schéma du subgraph et les entités indexées par le subgraph. Par conséquent, il est bon de définir le schéma du subgraph de manière à répondre aux besoins de votre dapp.
...
- Chaque type qui devrait être une entité doit être annoté avec une directive `@entity`.
- Par défaut, les entités sont mutables, ce qui signifie que les mappages peuvent charger des entités existantes, les modifier et stocker une nouvelle version de cette entité.
  - La mutabilité a un coût, donc pour les types d'entités qui ne seront jamais modifiés, comme ceux contenant des données extraites textuellement de la blockchain, il est recommandé de les marquer comme immuables avec `@entity(immutable: true)`.
...
L'entité `Gravatar` suivante est structurée autour d'un objet Gravatar et est un bon exemple de la manière dont une entité peut être définie.
...
Les exemples suivants `GravatarAccepted` et `GravatarDeclined` sont basés sur des événements. Il n'est pas recommandé de mapper des événements ou des appels de fonction aux entités de manière 1:1 .
...
| `Int8`       | Un entier signé 8 octets, également connu sous le nom d'entier signé 64 bits, peut stocker des valeurs dans la plage de -9,223,372,036,854,775,808 à 9,223,372,036,854,775,807. On préfèrera utiliser ceci pour représenter `i64` d'Ethereum. |
...
| `Timestamp`  | C'est une valeur `i64` en microsecondes. Généralement utilisé pour les champs `timestamp` des séries chronologiques et les agrégations.                                                                                                       |
...
Selon la spécification GraphQL, les commentaires peuvent être ajoutés au-dessus des attributs des entités du schéma en utilisant le symbole dièse (hashtag) `#`. Cela est illustré dans l'exemple ci-dessous :
...
Il existe une [bibliothèque Graph Typescript](https://github.com/graphprotocol/graph-tooling/tree/main/packages/ts) qui contient des utilitaires pour interagir avec le magasin Graph Node et des commodités pour manipuler les données et les entités des contrats intelligents. Elle peut être importée dans `mapping.ts` depuis `@graphprotocol/graph-ts`.
...
- Les [requêtes de voyage dans le temps](/querying/graphql-api/#time-travel-queries), qui permettent de requêter les états passés de ces entités à des blocs spécifiques tout au long de l'historique du subgraph
- Utiliser le subgraph comme une [base de greffe](/developing/creating-a-subgraph/#grafting-onto-existing-subgraphs) dans un autre subgraph, à ce bloc
...
Les entités de série chronologiques sont définies avec `@entity(timeseries: true)` dans schema.graphql. Chaque entité de séries chronologiques doit avoir un ID unique de type int8, un horodatage de type Timestamp, et inclure les données qui seront utilisées pour le calcul par les entités d'agrégation. Ces entités de Séries Chronologiques peuvent être sauvegardées dans des gestionnaires de déclencheurs réguliers et agissent comme les "données brutes" pour les entités d'Agrégation.

Les entités d'agrégation sont définies avec `@aggregation` dans schema.graphql. Chaque entité d'agrégation définit la source à partir de laquelle elle regroupera les données (qui doit être une entité de Séries chronologiques), fixe les intervalles (par exemple, heure, jour), et spécifie la fonction d'agrégation qu'elle utilisera (par exemple, somme, compte, min, max, premier, dernier). Les entités d'agrégation sont automatiquement calculées sur la base de la source spécifiée à la fin de l'intervalle requis.

#### Intervalles d'Agrégation disponibles

- `hour` : fixe la période des séries chronologiques chaque heure, à l'heure pile.
...
Ceci est similaire aux [modèles de sources de données existants](/developing/creating-a-subgraph/#data-source-templates), qui sont utilisés pour créer dynamiquement de nouvelles sources de données basées sur la blockchain.
...
> Vous pouvez utiliser des [filtres imbriqués](/querying/graphql-api/#example-for-nested-entity-filtering) pour filtrer les entités parents sur la base de ces entités imbriquées.
...
Le fichier source de données doit mentionner spécifiquement tous les types d'entités avec lesquels il interagira sous `entities`. Voir [limitations](#limitations) pour plus de détails.

#### Créer un nouveau gestionnaire pour traiter les fichiers

Ce gestionnaire doit accepter un paramètre de type `Bytes`, qui sera le contenu du fichier, lorsqu'il sera trouvé, et pourra ensuite être traité. Il s'agira souvent d'un fichier JSON, qui pourra être traité avec les assistants `graph-ts` ([documentation](/developing/graph-ts/api/#json-api)).
...
Vous pouvez utiliser le [contexte de DataSource ](/developing/graph-ts/api/#entity-and-datasourcecontext) lors de la création du File Data Sources pour transmettre des informations supplémentaires qui seront mises à la disposition du gestionnaire de la source de données de fichiers.
...
Les gestionnaires pour les File Data Sources ne peuvent pas être dans des fichiers qui importent des liaisons de contrats `eth_call`, échouant avec "unknown import: `ethereum::ethereum.call` has not been defined" ([issue](https://github.com/graphprotocol/graph-node/issues/4309)). La solution de contournement consiste à créer des gestionnaires de fichiers sources de données dans un fichier dédié.
