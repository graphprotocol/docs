...
- `laissez id = événement.transaction. hachage.concatI32(événement.logIndex.toI32())`
...
Il existe une [bibliothèque Graph Typescript](https://github.com/graphprotocol/graph-ts) qui contient des utilitaires pour interagir avec le magasin Graph Node et des commodités pour gérer les données et les entités des contrats intelligents. Elle peut être importée dans `mapping.ts` à partir de `@graphprotocol/graph-ts`.
...
> Dans ce contexte de subgraphs, le terme "historique" désigne le stockage de données reflétant les anciens états d'entités mutables. Cette capacité est essentielle pour les [requêtes de voyage dans le temps](/querying/graphql-api/#time-travel-queries), Cette fonctionnalité permet d'interroger les états passés de ces entités à des blocs spécifiques tout au long de l'histoire de la blockchain.
