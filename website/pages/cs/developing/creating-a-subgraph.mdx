...
> Abyste mohli svůj podgraf používat v decentralizované síti Graf, musíte si [vytvořit klíč API](/deploying/subgraph-studio-faqs/#2-how-do-i-create-an-api-key). Doporučujeme, abyste [přidali signál](/network/curating/#how-to-signal) do svého podgrafu s minimálně [3,000 GRT](/network-transition-faq/#how-can-i-ensure-that-my-subgraph-will-be-picked-up-by-indexer-on-the-graph-network).
...
Existuje [Knihovna Grafu Typescript](https://github.com/graphprotocol/graph-ts), která obsahuje nástroje pro interakci s úložištěm Uzel grafua vymoženosti pro práci s daty a entitami chytrých smluv. Lze jej importovat do `mapping.ts` z `@graphprotocol/graph-ts`.
...
> Pojem "historie" v tomto kontextu podgrafů znamená ukládání dat, která odrážejí staré stavy proměnlivých entit. Tato schopnost je nezbytná pro [dotazy na cestování v čase](/querying/graphql-api/#time-travel-queries), Tato funkce umožňuje dotazovat se na minulé stavy těchto entit v konkrétních blocích v celé historii blockchainu.
...
Je to podobné jako u [existujících šablon zdrojů dat](https://thegraph.com/docs/en/developing/creating-a-subgraph/#data-source-templates), které se používají k dynamickému vytváření nových řetězcových zdrojů dat.
...
> Pomocí vnořených filtrů můžete filtrovat nadřazené entity na základě těchto vnořených entit.</p> </blockquote> 
> 
...
> 
> Zdroj dat souboru musí v části `entity` výslovně uvádět všechny typy entit, se kterými bude spolupracovat. Další podrobnosti naleznete v části [omezení](#Limitations).
> 
> #### Vytvoření nové obslužné pro zpracování souborů
> 
> Tato obslužná rutina by měla přijmout jeden parametr `Bajtů`, což bude obsah souboru, pokud je nalezen, který pak může být zpracován. Často se bude jednat o soubor JSON, který lze zpracovat pomocí pomocných nástrojů `graph-ts` ([dokumentace](https://thegraph.com/docs/en/developing/assemblyscript-api/#json-api)).
> 
...
> 
> Pro Arweave může od verze 0.33.0 Graf Uzel načítat soubory uložené na Arweave na základě jejich [identifikátoru transakce](https://docs.arweave.org/developers/server/http-api#transactions) z brány Arweave ([vzor souboru](https://bdxujjl5ev5eerd5ouhhs6o4kjrs4g6hqstzlci5pf6vhxezkgaa.arweave.net/CO9EpX0lekJEfXUOeXncUmMuG8eEp5WJHXl9U9yZUYA)). Arweave podporuje transakce nahrané prostřednictvím Bundlr a Graf Uzel může také načítat soubory na základě [Bundlr manifestů](https://docs.bundlr.network/learn/gateways#indexing).
> 
> Příklad:
> 
> ```typescript
...
> 
> Při vytváření Souborových zdrojů dat můžete použít [Kontext zdroje dat](https://thegraph.com/docs/en/developing/assemblyscript-api/#entity-and-data-source-context) a předat tak další informace, které budou k dispozici obsluze Souborového zdroje dat.
> 
...
> 
> Obslužné rutiny pro zdroje dat souborů nemohou být v souborech, které importují vazby smlouvy `eth_call`, což se projeví chybou "neznámý import: `ethereum::ethereum.call` nebyl definován" ([problém](https://github.com/graphprotocol/graph-cli/issues/4309)). Řešením je vytvořit obsluhy zdrojů dat souborů ve vyhrazeném souboru.
> 
> #### Příklady
> 
