...
Prvním krokem k účasti v síti jako indexátor je schválení protokolu, vložení finančních prostředků a (volitelně) nastavení adresy operátora pro každodenní interakce s protokolem. _ **Poznámka**: Pro účely tohoto návodu bude pro interakci se smlouvou použit Remix, ale klidně použijte nástroj dle vlastního výběru ([OneClickDapp](https://oneclickdapp.com/), [ABItopic](https://abitopic.io/) a [MyCrypto](https://www.mycrypto.com/account) jsou některé další známé nástroje)._
...
3. Když je v editoru vybrán a otevřen soubor `GraphToken.abi`, přepněte se v rozhraní Remix na část Nasadit a `Spustit transakce`.
...
3. S vybraným a otevřeným souborem `Staking.abi` v editoru přejděte do sekce `Deploy` a `Run Transactions` v rozhraní Remix.
...
- **Aktivní** - Jakmile je alokace vytvořena v řetězci ([allocateFrom()](https://github.com/graphprotocol/contracts/blob/master/contracts/staking/Staking.sol#L873)), je považována za **aktivní**. Část vlastního a/nebo delegovaného podílu indexátoru je alokována na nasazení podgrafu, což mu umožňuje nárokovat odměny za indexaci a obsluhovat dotazy pro toto nasazení podgrafu. Agent indexátoru spravuje vytváření alokací na základě pravidel indexátoru.

- **Closed** - Indexer může alokaci uzavřít, jakmile uplyne 1 epocha ([closeAllocation()](https://github.com/graphprotocol/contracts/blob/master/contracts/staking/Staking.sol#L873)), nebo jeho agent indexeru alokaci automaticky uzavře po uplynutí **maxAllocationEpochs** (aktuálně 28 dní). Když je alokace uzavřena s platným dokladem o indexaci (POI), jsou jejich odměny za indexaci rozděleny agentu Indexer a jeho delegátům (více informací najdete v části "Jak se rozdělují odměny?" níže).
