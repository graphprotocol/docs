...
- Kdykoli budete potřebovat aktualizovat svůj podgraf, bude vám účtován poplatek za aktualizaci. Vzhledem k tomu, že je to spojeno s náklady, doporučujeme před nasazením do mainnet nasadit a otestovat subgraf na Goerli. V některých případech to může také vyžadovat určité GRT, pokud na daném subgrafu není žádný signál. V případě, že na dané verzi podgrafu signál/určení je (pomocí automatického migrování), budou daně rozděleny.
...
1. Proveďte změny v aktuálním podgrafu. Dobrým nápadem je otestovat malé opravy ve Studiu subgrafu zveřejněním v Goerli.
