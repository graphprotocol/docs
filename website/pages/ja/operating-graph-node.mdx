...
しかし、いくつかのインスタンスでは、イーサリアムノードがある期間に不正なデータを提供した場合、それがキャッシュに入り込み、不正なデータまたは失敗したサブグラフにつながる可能性があります。この場合、インデクサは `graphman` を使用して、汚染されたキャッシュをクリアし、影響を受けたサブグラフを巻き戻して、（できれば）健全なプロバイダから新しいデータをフェッチすることができます。
...
しかし、専用のクエリノードとレプリカを使用しても、特定のクエリの実行に時間がかかり、場合によってはメモリの使用量が増えて、他のユーザーのクエリ時間に悪影響を与えることがあります。
