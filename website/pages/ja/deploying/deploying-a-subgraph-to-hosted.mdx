...
ここで、サブグラフを`mainnet`と`sepolia`ネットワークにデプロイできるようにしたいと仮定し、これが`subgraph.yaml`であるとしましょう:
...
`build`コマンドは、`subgraph.yaml`を`sepolia`の設定で更新し、サブグラフをリコンパイルします。`subgraph.yaml`ファイルは、以下のようになります:
...
この方法を説明するために、あるサブグラフが異なるコントラクトアドレスを使用してmainnetと Sepoliaにデプロイされるべきだと仮定してみましょう。その場合、各ネットワークのアドレスを提供する 2 つのconfigファイルを定義することができます。
...
このサブグラフをmainnetやSepoliaにデプロイするには、次の 2 つのコマンドのどちらかを実行するだけです:
