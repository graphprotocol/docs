...
[Substreams](/substreams/README) は、Graph Network 向けに StreamingFast によって開発された、ブロックチェーン データを処理するための新しいフレームワークです。サブストリーム モジュールは、サブグラフ エンティティと互換性のあるエンティティの変更を出力できます。サブグラフは、このようなサブストリーム モジュールをデータ ソースとして使用でき、サブストリームのインデックス作成速度と追加データをサブグラフ開発者にもたらします。
...
Substreams パッケージは、タイプ ([プロトコル バッファー](https://protobuf.dev/) として定義)、モジュール (Rust で記述)、およびタイプを参照し、モジュールの使用方法を指定する `substreams.yaml` ファイルで構成されます。 が引き起こされます。 [サブストリーム開発の詳細については、サブストリームのドキュメントにアクセスしてください](/substreams/README)、[awesome-substreams](https://github.com/pinax-network/awesome-substreams) および [サブストリーム クックブック] を確認してください。 その他の例については、(https://github.com/pinax-network/substreams-cookbook) を参照してください。
