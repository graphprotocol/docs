...
Please note that this indexing incentive does not deplete over time; it has no depletion rate and is instantly withdrawable at any time. The only current cost is a 1% curation fee when a user signals on the subgraph. If you recently created a subgraph and signaled it with 3000 GRT, you would need an additional 30 GRT for the curation fee (which would be burned). Note that a 0.5% fee is also taken every time a new deployment is made with a new subgraph version.
