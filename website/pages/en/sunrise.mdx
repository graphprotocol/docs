...
Hosted service subgraphs will no longer be available after June 12th, 2024 (the cutoff date). Developers will no longer be able to deploy new versions, and query endpoints will be heavily rate limited, returning an error message explaining that the hosted service is no longer available, and what to do next.
...
Subgraphs will be queryable on the hosted service until the cutoff date, after which queries will be heavily rate limited, and owners will not be able to deploy new versions. Owners will still be able to simply upgrade their subgraphs to the network at any time, even after the cutoff date.
