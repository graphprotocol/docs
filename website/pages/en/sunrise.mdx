...
Hosted service subgraphs will be deprecated on June 12th, 2024 (the cutoff date). Developers will no longer be able to deploy new versions, and query endpoints will be heavily rate limited, returning an error message explaining that the hosted service is no longer available, and what to do next.
