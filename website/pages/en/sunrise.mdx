...
All subgraphs will be supported by the upgrade Indexer. For a higher quality of service and more robust redundancy, you can add a curation signal to subgraphs eligible for indexing rewards. At the time of writing, it is recommended that you curate your subgraph with at least 3,000 GRT (per subgraph) to attract about 3 Indexers. [Learn more about adding signal to your subgraph](/publishing/publishing-a-subgraph/#adding-signal-to-your-subgraph).

Please note that this indexing incentive does not deplete over time; it has no depletion rate and is instantly withdrawable at any time. The only cost at the moment is a 1% curation fee when a user signals on the subgraph. If you recently created a subgraph and signaled it with 3,000 GRT, you would need an additional 30 GRT for the curation fee (which would be burned). Note that a 0.5% fee is also taken every time a new deployment is made with a new subgraph version.
...
### What should I do with my old subgraphs? Will they stop working and should I delete them?

It is not possible to delete subgraphs, but they can be deprecated. To learn how to deprecate a subgraph, check out [deprecating a subgraphs](https://thegraph.com/docs/en/managing/deprecating-a-subgraph/). Query endpoints will no longer be available, and owners will not be able to deploy new subgraphs to the hosted service.

### Will Subgraph Studio be impacted by this upgrade?

No, Subgraph Studio will not be impacted by Sunrise.

### Will the hosted service be closed?

Yes. Query endpoints will no longer be available, and owners will not be able to deploy or query the hosted service.

### Will subgraphs need to be re-indexed again?

No, rest assured that your subgraph will not need to be re-indexed when it is upgraded from the hosted service to Subgraph Studio. To get additional indexing support, publish your subgraph to The Graph Network, where new Indexers will sync your subgraph. In the meantime, the upgrade Indexer will seamlessly support your existing subgraphs.

### Why do I publish my subgraph to Arbitrum?

The Graph protocol originally lived on Ethereum but moved to Arbitrum to reduce gas costs for all users. Subgraphs are published to The Graph on Arbitrum so that they can be indexed by Indexers engaging with smart contracts on that chain. Arbitrum is the network that subgraphs are deployed to, but subgraphs can index any of the [supported networks](https://thegraph.com/docs/en/developing/supported-networks/)
