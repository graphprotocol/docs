...
To initialize a subgraph that tracks a Proxy contract. You need to initialzie the subgraph with the implementation contract address. After the initialization wizard is complete, you can update the `subgraph.yaml` file to point to the proxy contract address.
...
## Supported Networks on the Hosted Service
>>>>>>> 7c14edf (Add initializing subgraphs indexing Proxy Contracts)
