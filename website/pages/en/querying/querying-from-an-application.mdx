...
Once a subgraph is deployed to [Subgraph Studio](https://thegraph.com/studio/) or to [Graph Explorer](https://thegraph.com/explorer), you will be given the endpoint for your GraphQL API.
...
### Graph Client Overview

The Graph is provides its own `graph-client` which supports unique features such as:
...
Popular Graph QL clients such as Apollo and URQL are compatible with environments such as React, Angular, Node.js, and React Native. As a result, using `graph-client` will provide an optimal experience for working with The Graph.
