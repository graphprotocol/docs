...
The first time, it would send the query with `lastID = ""`, and for subsequent requests would set `lastID` to the `tokenId` attribute of the last entity in the previous request. This approach will perform significantly better than using increasing `skip` values.
