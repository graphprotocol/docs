...
- **Indexing Reward Cut** - The portion of the rewards the Indexer will keep for themselves.
  - If an Indexer's rewards are set to 100%, as a Delegator, you will get 0 indexing rewards.
  - If you see it set at 80% , as a Delegator, you will receive 20%.
...
- **Query Fee Cut** - This is the same as the Indexing Reward Cut, but it specifically applies to returns on the query fees the Indexer collects.
...
- Many of them have been Indexing for month, and they are doing their best to help Delegators earn a good return, as it improves the health and success of the network.
...
You should choose an Indexer that is transparent and honest about setting their Query Fee Cut and Indexing Fee Cuts. You should also look at the Parameters Cooldown time to see how much of a time buffer you have. After that is done, it is simple to calculate the amount of rewards you are getting.
...
> The formula above shows that it is possible for an Indexer offering only 20% to Delegators can provide a better reward than an Indexer giving 90%. Simply do the math to determine the best reward.
