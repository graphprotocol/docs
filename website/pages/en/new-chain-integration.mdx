...
> Note that while the recommended approach is to develop a new Firehose for all new chains, it is only required required for non-EVM chains.
...
It is really a matter of where you put your logic, in the subgraph or the Substreams. Keep in mind that having your logic in Substreams benefits from a parallelized model, whereas Substreams triggers will be linearly consumed. Consider the following example implementing a subgraph handler:
