...
- All those `getLogs` calls and roundtrips get replaced by a single stream arriving into the heart of `graph-node` and by running filters/transformations (Substreams) on the in-memory blocks; a single block model for all subgraphs it processes.
