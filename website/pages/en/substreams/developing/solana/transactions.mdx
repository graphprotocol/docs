...
To make your Substreams queryable (as opposed to [direct streaming](https://docs.substreams.dev/how-to-guides/sinks/stream)), you can automatically generate a [Substreams-powered subgraph](../../../sps/introduction.mdx) or SQL-DB sink.

### Subgraph

1. Run `substreams codegen subgraph` to initialize the sink, producing the necessary files and function definitions.
2. Create your [subgraph mappings](../../../sps/triggers.mdx) within the `mappings.ts` and associated entities within the `schema.graphql`.
3. Build and deploy locally or to Studio by running `deploy-studio`.
...
- The [Dev Container Reference](../devcontainer.mdx) helps you navigate the container and its common errors.
