...
> Note: This guide excludes [Account Changes](substreams/developing/solana/accountchanges/).
...
2. Running `substreams init` will give you the option to choose between two Solana project options. Select the best option for your project:
   ...
   - **sol-anchor-beta**: This creates a Substreams that decodes instructions and events with an Anchor IDL. If an IDL isn’t available, after using the `idl` subcommand within the [Anchor CLI](https://www.anchor-lang.com/docs/cli), then you’ll need to provide it yourself.
...
To make your Substreams queryable (as opposed to [direct streaming](../how-to-guides/sinks/stream/stream.md)), you can automatically generate a [Substreams-powered subgraph](/substreams/developing/sinks/sps/introduction/) or SQL-DB sink.
...
2. Run `substreams build` build the [Substream SQL](/how-to-guides/sinks/sql/sql-sink.md) sink.
