...
> This cookbook uses this [Substreams-powered subgraph as a reference](https://github.com/graphprotocol/graph-tooling/tree/azf20/substreams-powered-subgraph-example/examples/substreams-powered-subgraph).

## Requirements

This cookbook [requires the dependencies necessary for local Substreams development](https://substreams.streamingfast.io/developers-guide/installation-requirements), as well as Graph CLI >=0.51.1:
...
The Substreams package in question detects contract deployments on Mainnet Ethereum, tracking the creation block and timestamp for all newly deployed contracts. To do this, there is a dedicated `Contract` type in `/proto/example.proto`:
...
To prepare the Substreams package for consumption by a subgraph, you must run the following commands:

```bash
cargo protogen # generates types in /src/pb
cargo build # builds the substreams
cargo package # packages the substreams in a .spkg file
```
...
type Contract @entity {
  ...
  timestamp: String! #timestamp of when the contract was deployed
  blockNumber: BigInt! #block number of the contract deployment
}
```
...
graph deploy --studio subgraph-name-here # deploy the subgraph to the Subgraph Studio
```
