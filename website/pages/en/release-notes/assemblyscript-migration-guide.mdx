...
```typescript
const maybeValue = load()! // breaks in runtime if value is null
...
```typescript
const byteArray = new ByteArray(10)
const uint8Array = byteArray as Uint8Array // equivalent to: <Uint8Array>byteArray
```
...
```typescript
// remove nullability
const previousBalance = AccountBalance.load(balanceId) // AccountBalance | null
...
```typescript
const something: string | null = 'data'
...
const somethingOrElse: string = container.data ? container.data : 'else' // doesn't compile
```
...
const somethingOrElse: string = data ? data : 'else' // compiles just fine :)
```
...
const x = BigInt.fromI32(2)
const y: bigint | null = null

console.log(x + y) // give compile time error about nullability

const wrapper = new Wrapper(y)
...
```typescript
const value = new Type() // initialized
value.x = 10
value.y = 'content'
```
