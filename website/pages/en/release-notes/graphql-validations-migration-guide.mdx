...
fragment MyFieldsMetadata on MyType { # assign a unique name to fragment
  metadata
}

fragment MyFieldsName on MyType { # assign a unique name to fragment
  name
}
```
...
fragment AllFields on MyType { # unused :(
  name
  age
}
```
