...
Here is a table showing the networks that The Graph supports, whether it is on the hosted service, Subgraph Studio, or the decentralized network:
...
<Table>
  ...
  {getSupportedNetworks().map((network) => (
    <tr key={network.cliName}>
      ...
      <td align="center">{network.supportedOnNetwork ? '✓' : null}</td>
    </tr>
  ))}
</Table>
