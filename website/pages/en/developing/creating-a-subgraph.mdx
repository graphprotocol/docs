...
const tradingPair = context.getString('tradingPair')
```
...
export function handleCreateGravatar(call: CreateGravatarCall): void {
  const id = call.transaction.hash
...
export function handleMetadata(content: Bytes): void {
  const tokenMetadata = new TokenMetadata(dataSource.stringParam())
