...
When creating and saving a new entity, if an entity with the same ID already exists, the properties of the new entity are preferred during the merge process. This means that the existing entity will be updated with the values from the new entity. However, if the new entity contains null fields, these fields will not overwrite the corresponding fields in the existing entity. Instead, the non-null fields from the new entity are merged with the existing entity, preserving any data in fields that were null in the new entity. This behavior ensures that data is not unintentionally lost during the update process.
