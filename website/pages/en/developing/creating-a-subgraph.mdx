...
eventHandlers:
  - event: Transfer(indexed address,indexed address,uint256)
    ...
    topic1: ['0xAddressA', '0xAddressB', '0xAddressC'] # Sender or Receiver Address
    topic2: ['0xAddressB', '0xAddressC'] # Sender or Receiver Address
```

In this configuration:

- `topic1` is configured to filter `Transfer` events where `0xAddressA`, `0xAddressB`, `0xAddressC` is either the sender or the receiver.
- `topic2` is configured to filter `Transfer` events where `0xAddressB` and `0xAddressC` is either the sender or the receiver.
