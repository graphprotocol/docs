...
```typescript
const id = event.transaction.hash // or however the ID is constructed
...
export function handleTransfer(event: TransferEvent) {
  ...
  const contract = ERC20Contract.bind(event.address)

  // Access state variables and functions by calling them
  const erc20Symbol = contract.symbol()
}
```
...
```typescript
const gravity = Gravity.bind(event.address)
const callResult = gravity.try_gravatarToOwner(gravatar)
if (callResult.reverted) {
  log.info('getGravatar reverted', [])
} else {
  const owner = callResult.value
}
```
...
const decoded = ethereum.decode('(address,uint256)', encoded)
```
...
const hash = 'QmTkzDwWqPbnAh5YiV5VwcTLnGdwSNsNTn2aDxdXBFca7D'
...
const path = 'QmTkzDwWqPbnAh5YiV5VwcTLnGdwSNsNTn2aDxdXBFca7D/Makefile'
