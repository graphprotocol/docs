...
The store API facilitates the retrieval of entities that were created or updated in the current block. This feature proves to be useful in situations where the handler requires the on-chain event to be generated in the same block. By utilizing this functionality, indexing performance can improve significantly.
