...
import { handleNewGravatar, handleUpdatedGravatar } from '../../src/gravity'

beforeAll(() => {
  const gravatar = new Gravatar('0x0')
...
import { afterAll, describe, test } from 'matchstick-as/assembly/index'
...
```typescript
import { afterAll, clearStore, describe, test } from 'matchstick-as/assembly/index'
...
```typescript
import { beforeEach, describe, test } from 'matchstick-as/assembly/index'
...
```typescript
import { afterEach, beforeEach, describe, test } from 'matchstick-as/assembly/index'
...
test('Can call mappings with custom events', () => {
  ...
  // Create mock events
  const newGravatarEvent = createNewGravatarEvent(12345, '0x89205A3A3b2A69De6Dbf7f01ED13B2108B2c43e7', 'cap', 'pac')
  const anotherGravatarEvent = createNewGravatarEvent(3546, '0x89205A3A3b2A69De6Dbf7f01ED13B2108B2c43e7', 'cap', 'pac')
...
```typescript
const gravatar = new Gravatar('entryId')
gravatar.save()
```
...
import { createNewGravatarEvent, handleNewGravatars } from './mapping'
...
import { addMetadata, assert, clearStore, createMockedFunction, test } from 'matchstick-as/assembly/index'
...
const gravity = Gravity.bind(contractAddress)
const result = gravity.gravatarToOwner(bigIntParam)
...
import { assert, mockIpfsFile, test } from 'matchstick-as/assembly/index'
...
```typescript
import { Address, Bytes, ethereum, ipfs, json, JSONValue, Value } from '@graphprotocol/graph-ts'
...
export function gravatarFromIpfs(): void {
  ...
  const url = jsonData.get('imageUrl')
...
```typescript
// Read
const logType = newGravatarEvent.logType
...
test('Derived fields example test', () => {
  ...
  mainAccount.save()
  const operatedAccount = new GraphAccount('1')
  operatedAccount.operators = ['12']
  operatedAccount.save()
  const nst = new NameSignalTransaction('1234')
  nst.signer = '12'
  nst.save()
...
export function handleApproveTokenDestinations(event: ApproveTokenDestinations): void {
  const tokenLockWallet = TokenLockWallet.load(dataSource.address().toHexString())!
...
import { assert, dataSourceMock, newMockEvent, test } from 'matchstick-as/assembly/index'
...
test('Data source simple mocking example', () => {
  const addressString = '0xA16081F360e3847006dB660bae1c6d1b2e17eC2A'
  ...
  const event = changetype<ApproveTokenDestinations>(newMockEvent())
