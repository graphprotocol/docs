...
Sin embargo, en algunos casos, si un nodo de Ethereum ha proporcionado datos incorrectos durante algún tiempo, estos pueden introducirse en el caché, dando lugar a datos incorrectos o subgrafos fallidos. En este caso, los Indexadores pueden utilizar `graphman` para limpiar la caché envenenada y, a continuación, rebobinar los subgrafos afectados, que recuperarán datos frescos del proveedor (esperemos) sano.
...
Sin embargo, incluso con un nodo de consulta dedicado y réplicas, ciertas consultas pueden tardar mucho tiempo en ejecutarse y, en algunos casos, aumentar el uso de memoria y repercutir negativamente en el tiempo de consulta de otros usuarios.
