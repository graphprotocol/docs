...
Los desarrolladores representan el lado de la demanda del ecosistema The Graph. Los desarrolladores construyen subgrafos y los publican en The Graph Network. A continuación, consultan los subgrafos activos con GraphQL para potenciar sus aplicaciones.
...
> Existen ciertas limitaciones en The Graph Network, en términos de features y soporte de red. Actualmente, solo los subgrafos de la mainnet de Ethereum obtienen recompensas de indexación, y los subgrafos que obtienen datos de IPFS tampoco son elegibles.

#### Deploy en el Subgraph Studio
...
Para poder realizar consultas, los desarrolladores deben generar una clave de API, que se puede hacer en el Subgraph Studio. Esta clave API debe ser provista de fondos con GRT, para poder pagar las tarifas de consulta. Los desarrolladores pueden establecer una tarifa de consulta máxima, para controlar sus costes, y limitar su clave API a un determinado subgrafo o dominio de origen. Subgraph Studio proporciona a los desarrolladores datos sobre el uso de su clave API a lo largo del tiempo.
...
#### Actualización de subgrafos

Después de un tiempo, un desarrollador de subgrafos puede querer actualizar su subgrafo, quizás corrigiendo un error o añadiendo nuevas funcionalidades. El desarrollador de subgrafos puede deploy una o varias nuevas versiones de sus subgrafos en el Subgraph Studio para el desarrollo y las pruebascon tarifas limitadas.

Una vez que el desarrollador de subgrafos está listo para actualizar, puede iniciar una transacción para apuntar su subgrafo a la nueva versión. La actualización del subgrafo migra cualquier señal a la nueva versión (suponiendo que el usuario que aplicó la señal seleccionó "migrar automáticamente"), lo que también incurre en un impuesto de migración. Esta migración de señales debería dar pie a que los Indexadores comiencen a indexar la nueva versión del subgrafo, por lo que pronto debería estar disponible para realizar consultas.

#### Subgrafos deprecados

En algún momento un desarrollador puede decidir que ya no necesita un subgrafo publicado. En ese momento pueden deprecar el subgrafo, lo que devuelve cualquier GRT señalada a los Curadores.
...
## Developers y economía de la red
