...
> **注意：** 投票筛选器仅适用于`kind: ethereum`的数据源。
...
> **注意：** 一次性筛选器仅适用于`kind: ethereum`的数据源。
...
| 特征                                      | 名称                                                |
| ----------------------------------------- | --------------------------------------------------- |
| [非致命错误](#非致命错误)                 | `nonFatalErrors`                                    |
| [全文搜索](#定义全文搜索字段)             | `fullTextSearch`                                    |
| [嫁接](#嫁接到现有子图)                   | `grafting`                                          |
| [以太坊合约上的IPFS](#以太坊合约上的IPFS) | `ipfsOnEthereumContracts` or `nonDeterministicIpfs` |
...
> **注意：** 在初次升级到The Graph Network时，不建议使用grafting。可以在[这里](/cookbook/grafting/#important-note-on-grafting-when-upgrading-to-the-network)了解更多信息。
...
对于IPFS，Graph Node支持[v0和v1内容标识符](https://docs.ipfs.tech/concepts/content-addressing/)，以及带有目录的内容标识符（例如`bafyreighykzv2we26wfrbzkcdw37sbrby4upq7ae3aqobbq7i4er3tnxci/metadata.json`）。
