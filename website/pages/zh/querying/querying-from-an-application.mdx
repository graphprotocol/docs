...
让我们看看如何使用 `graphql-client` 从子图中获取数据。
...
运行以下 Graph Client CLI 命令将生成类型化的代码并准备使用 JavaScript 代码:
...
`graph-client` 与其他 GraphQL 客户端(如 Apollo client、 URQL 或 React Query)完美地集成在一起; 您可以在[正式存储库中找到示例](https://github.com/graphprotocol/graph-client/tree/main/examples)。
