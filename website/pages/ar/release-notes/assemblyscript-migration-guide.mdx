...
2. قم بتحديث `graph-cli` الذي تستخدمه إلى `latest` عن طريق تشغيل:
...
3. افعل الشيء نفسه مع `graph-ts` ، ولكن بدلاً من التثبيت بشكل عام ، احفظه في التبعيات الرئيسية:
...
هناك سيناريوهين قد ترغب في ال cast ، ولكن استخدام `as`/`<T>var` **<strong x-id="1> ليس آمنا **:
...
أضفنا أيضًا بعض ال طرق ثابتة في بعض الأنواع وذلك لتسهيل عملية ال Casting ، وهي:
