...
## 6. Quais são as diferenças entre modelos e fontes de dados?
...
## 7. Como garantir que estou a usar a versão mais recente do graph-node para os meus lançamentos locais?
...
## 8. Como chamo uma função de contrato ou acedo a uma variável de estado público dos meus mapeamentos de subgraph?
...
## 9. É possível preparar um subgraph através de `graph init`, a partir da `graph-cli` com dois contratos? Ou devo adicionar, manualmente, outra fonte de dados no `subgraph.yaml` após executar a `graph init`?
...
## 10. Quero contribuir ou adicionar um problema no GitHub. Onde posso encontrar os repositórios de código aberto?
...
## 11. Qual é a forma recomendada de construir ids "autogeradas" para uma entidade ao lidar com eventos?
...
## 12. Ao escutar vários contratos, é possível selecionar a ordem do contrato para escutar eventos?
...
## 13. É possível diferenciar entre redes (mainnet, Sepolia, local) de dentro de handlers de eventos?
...
## 14. Vocês apoiam handlers de bloco e de chamadas no Sepolia?
...
## 15. Posso importar ethers.js ou outras bibliotecas JS nos meus mapeamentos de subgraph?
...
## 16. É possível especificar em qual bloco começar a indexação?
...
## 17. Alguma dica para aumentar o desempenho da indexação? O meu subgraph demora muito para sincronizar
...
## 18. Há como consultar diretamente o subgraph para determinar o número do último bloco que ele indexou?
...
## 19. Quais redes são apoiadas pelo The Graph?

Veja a lista das redes apoiadas [aqui](/developing/supported-networks).

## 20. É possível duplicar um subgraph para outra conta ou outro endpoint sem relançá-lo?

Deve relançar o subgraph, mas se a ID do subgraph (hash IPFS) não mudar, ele não precisará sincronizar do começo.

## 21. É possível usar a Apollo Federation juntamente ao graph-node?
...
## 23. Se a frontend do meu dApp usa o The Graph para consultas, eu preciso escrever a minha chave de query diretamente no frontend? E se pagarmos taxas de query para utilizadores — utilizadores maliciosos podem aumentar demais estas taxas?
...
## 24. Como atualizar um subgraph na mainnet?
...
## 25. Em qual ordem os handlers de evento, bloco, e chamada são ativados para uma fonte de dados?
