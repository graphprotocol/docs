...
Se o seu subgraph continuar a falhar, faça um query sobre a saúde do subgraph com o GraphiQL Playground. Perceba que dá para usar a consulta abaixo e inserir sua ID de lançamento para seu subgraph. Neste caso, `Qm...` é a ID de lançamento (na página Subgraph sob **Detalhes**). O texto abaixo te dirá quando um subgraph falha para que você possa fazer o debug de acordo:
...
Selecione a rede onde quer editar o seu subgraph. É bom editar subgraphs no Arbitrum One, para aproveitar as [transações mais rápidas e custos menores de gas](/arbitrum/arbitrum-faq).
