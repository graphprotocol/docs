...
Para integrações de Firehose/Substreams lideradas pela StreamingFast, são inclusos um apoio básico para módulos fundamentais de Substreams (por ex., transações decodificadas, logs e eventos de contratos inteligentes) e ferramentas de codegen para subgraphs movidos a Substreams (confira o [Injective](https://substreams.streamingfast.io/documentation/intro-getting-started/intro-injective/injective-first-sps) para um exemplo).

Há duas opções para consumir dados em Substreams via um subgraph:

- **Gatilhos de Substreams:** Consuma a partir de qualquer módulo de Substreams ao importar o modelo Protobuf via um handler de subgraph, e migre toda a sua lógica até um subgraph. Este método cria as entidades de subgraph diretamente no próprio subgraph.
- **EntityChanges:** Ao escrever mais da lógica nos Substreams, é possível consumir o resultado do módulo diretamente no `graph-node`. No `graph-node`, o programador pode usar os dados dos Substreams para criar as suas entidades de subgraph.

É só uma questão de onde colocar a sua lógica: no subgraph ou nos Substreams. Lembre-se que colocar mais da sua lógica nos Substreams se beneficia de um modelo paralelizado, enquanto gatilhos serão consumidos de forma linear no `graph-node`. Considere o exemplo a seguir ao implementar um handler de subgraph:
...
A função `handleTransactions` é um handler de subgraph que recebe os bytes brutos de Substreams como um parâmetro e os decodifica num objeto `Transactions`. Depois, para cada transação, é criada uma nova entidade de subgraph. Para mais informações sobre gatilhos de Substreams, visite a [documentação da Streamingfast](https://substreams.streamingfast.io/documentation/consume/subgraph/triggers) ou confira módulos de comunidade no [substreams.dev](https://substreams.dev/).
