...
O Substreams é uma tecnologia poderosa de indexação de blockchains, desenvolvida pela Graph Network. Ele permite que programadores escrevam módulos em Rust, componham transmissões de dados ao lado da comunidade, e providencia indexações de altíssimo desempenho por virtude da paralelização, de uma forma que prioriza o streaming.

Com o Substreams, programadores podem extrair rapidamente dados de blockchains diferentes (Ethereum, BNB, Solana, etc.) e enviá-los para vários locais da sua escolha, como um banco de dados Postgres, um banco de dados Mongo, ou um Subgraph. Além disto, pacotes de Substreams permitem que programadores especifiquem quais dados eles querem extrair de uma blockchain.
