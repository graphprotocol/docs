...
O parâmetro `where` pode ser usado nos seus queries para filtrar buscas por propriedades diferentes. É possível filtrar em múltiplos valores no parâmetro `where`.
