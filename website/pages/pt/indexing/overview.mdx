...
1. Faça um query do [subgraph da mainnet](https://thegraph.com/explorer/subgraphs/9Co7EQe5PgW3ugCUJrJgRv4u9zdEuDJf8NvMWftNsBH8?view=Query&chain=arbitrum-one) para buscar as IDs para todas as alocações ativas:
...
Os valores `queryFeeCut` e `indexingRewardCut` são parâmetros de delegação que o Indexador pode configurar, junto do `cooldownBlocks`, para controlar a distribuição de GRT entre o Indexador e os seus Delegantes. Veja os últimos passos em [Staking no Protocolo](/indexing/overview/#stake-in-the-protocol) para instruções sobre como configurar os parâmetros de delegação.
...
- **Sinal de curadoria** — A proporção do sinal de curadoria na rede aplicado a um subgraph particular mede bem o interesse naquele subgraph; especialmente durante a fase de inicialização, quando o volume de queries começa a subir.
...
- **Quantidade em staking** — Ao monitorizar o comportamento de outros Indexadores ou inspecionar proporções de stake total alocados a subgraphs específicos, um Indexador pode monitorizar a reserva para queries nos subgraphs, para identificar subgraphs nos quais a rede mostra confiança ou subgraphs que podem necessitar de mais reservas.

- **Subgraphs sem recompensas de indexação** — Alguns subgraphs não geram recompensas de indexação, principalmente por usarem recursos não apoiados — como o IPFS — ou porque consultam outra rede fora da mainnet. Se um subgraph não estiver a gerar recompensas de indexação, o Indexador será notificado a respeito.
...
- **Firewall** — O serviço de Indexadores é o único que precisa ser exposto publicamente. O trancamento de portas de admin, e o acesso ao banco de dados, exigem muito mais atenção: o endpoint JSON-RPC do Graph Node (porta padrão: 8030), o endpoint da API de gestão do Indexador (porta padrão: 18000), e o endpoint do banco de dados Postgres (porta padrão: 5432) não devem ser expostos.
...
- **Endpoint de dados** — Para redes compatíveis com EVMs, o Graph Node deve estar conectado a um endpoint que expõe uma API JSON-RPC compatível com EVMs. Isto pode ser um único cliente, ou um setup mais complexo que carrega saldos em várias redes. É importante saber que certos subgraphs exigirão capacidades particulares de clientes, como um modo de arquivo e uma API de rastreamento.

- **Node IPFS (versão menor que 5)** — Os metadados de lançamento de subgraph são armazenados na rede IPFS. O Graph Node acessa primariamente o node IPFS durante o lançamento do subgraph, para retirar o manifest e todos os arquivos ligados. Indexadores de rede não precisam hospedar o seu próprio node IPFS, pois já há um hospedado para a rede em https://ipfs.network.thegraph.com.

- **Serviço de Indexador** — Cuida de todas as comunicações externas com a rede necessárias. Divide modelos de custo e estados de indexação, passa pedidos de consulta de gateways para um Graph Node, e monitora os pagamentos de consulta via canais de estado com o gateway.
...
| Porta | Propósito                  | Rotas  | Argumento na CLI              | Variáveis de ambiente                    |
| ----- | -------------------------- | ------ | ----------------------------- | ---------------------------------------- |
...
- Clone, ou navegue para, o [repositório do Indexer](https://github.com/graphprotocol/indexer).

- Navegue ao diretório `./terraform`, que é onde todos os comandos devem ser executados.
...
# Isto resgata o e-mail da conta de serviço da lista
...
- Leia todos os arquivos no `$dir` e ajuste quaisquer valores conforme indicado nos comentários.

Lance todos os recursos com `kubectl apply -k $dir`.
...
- **Node Ethereum** — Por padrão, o setup do docker utilizará a mainnet: [http://host.docker.internal:8545](http://host.docker.internal:8545) para se conectar ao node Ethereum na sua máquina de hospedagem. Este nome e url podem ser substituídos com uma atualização no `docker-compose.yaml`.
...
- **Serviço indexador** — O único componente que deve ser exposto externamente, o serviço passa queries de subgraph ao graph node, cuida de canais de estado para pagamentos de consulta, e compartilha informações importantes de decisões para clientes, como os gateways.

- **Indexer CLI** — A interface de comando de linha para manusear o agente indexador. Ela permite que Indexadores cuidem de modelos de custo, alocações manuais, filas de ações e regras de indexação.
...
\*\*NOTA: Após iniciar os containers, o serviço Indexador deve ser acessível no [http://localhost:7600](http://localhost:7600) e o agente indexador deve expor a API de gestão no [http://localhost:18000/](http://localhost:18000/).

#### Usando K8s e Terraform

Veja a secção [Como Organizar a Infraestrutura de um Servidor com o Terraform na Google Cloud](/indexing/overview/#setup-server-infrastructure-using-terraform-on-google-cloud)

#### Uso

> **NOTA:** Todas as variáveis de configuração de runtime (tempo de execução) podem ser aplicadas ao comando como parâmetros, na inicialização, ou usando variáveis de ambiente do formato `COMPONENT_NAME_VARIABLE_NAME` (por ex. `INDEXER_AGENT_ETHEREUM`).
...
O Indexer CLI é um plugin para [`@graphprotocol/graph-cli`](https://www.npmjs.com/package/@graphprotocol/graph-cli), acessível pelo terminal em `graph indexer`.
...
- `graph indexer connect <indexer-agent-pod>` — Conecta à API de gestão de Indexador. Tipicamente, a conexão ao servidor é aberta através do redirecionamento de portas, para que a CLI possa ser operada remotamente com facilidade. (Exemplo: `kubectl port-forward pod/<indexer-agent-pod> 8000:8000`)

- `graph indexer rules get [options] <deployment-id> [<key1>...]` — Mostra uma ou mais regras de indexação usando `all` como o `<deployment-id>` para mostrar todas as regras, ou global para exibit os padrões globais. Um argumento adicional `--merged` pode ser usado para especificar que regras específicas ao lançamento serão fundidas com a regra global. É assim que elas são aplicadas no agente de Indexador.

- `graph indexer rules set [options] <deployment-id> <key1> <value1> ...` — Configura uma ou mais regras de indexação.

- `graph indexer rules start [options] <deployment-id>` — Começa a indexar um lançamento de subgraph, se disponível, e configura a sua `decisionBasis` para `always`, fazendo que o agente indexador sempre escolha indexá-lo. Caso a regra global seja configurada para `always`, todos os subgraphs disponíveis na rede serão indexados.

- `graph indexer rules stop [options] <deployment-id>` — Para de indexar um lançamento e configura a sua `decisionBasis` para `never`, com o fim de pular este lançamento ao decidir quais lançamentos indexar.

- `graph indexer rules maybe [options] <deployment-id>` — Configura a `decisionBasis` de um lançamento para `rules`, comandando o agente indexador a usar regras de indexação para decidir se este lançamento será ou não indexado.

- `graph indexer actions get [options] <action-id>` — Retira uma ou mais ações usando o `all`, ou deixa o `action-id` vazio para mostrar todas as ações. Um argumento adicional — `--status` — pode ser usado para imprimir todas as ações de um certo status.

- `graph indexer action queue reallocate <deployment-id> <allocation-amount>` — Programa uma ação de realocação
...
Por exemplo: se a regra global tem um `minStake` de **5** (GRT), qualquer lançamento de subgraph que tiver mais de 5 (GRT) de stake alocado nele será indexado. As regras de limiar incluem `maxAllocationPercentage`, `minSignal`, `maxSignal`, `minStake`, e `minAverageQueryFees`.
...
- Se uma ação tiver êxito, o programa garantirá a presença de uma regra de indexação que diga ao agente como administrar a alocação dali em diante. Isto será mais conveniente para executar ações manuais enquanto o agente está no modo `auto` ou `oversight`.
...
```
# Esta instrução captura o valor de skip,
...
# Este padrão será igualado a qualquer expressão na GraphQL.
# Ele usa um Global, inserido na expressão para calcular o custo
...
> Nota: Para os fins destas instruções, o Remix será usado para interação com contratos, mas é possível escolher a sua ferramenta preferida (Por exemplo: [OneClickDapp](https://oneclickdapp.com/), [ABItopic](https://abitopic.io/) e [MyCrypto](https://www.mycrypto.com/account)).
...
1. Abra o [aplicativo Remix](https://remix.ethereum.org/) em um navegador de Internet

2. Em `File Explorer`, crie um arquivo chamado **GraphToken.abi** com o [token ABI](https://raw.githubusercontent.com/graphprotocol/contracts/mainnet-deploy-build/build/abis/GraphToken.json).

3. Com `GraphToken.abi` selecionado e aberto no editor, abra a seção `Deploy and Run Transactions` (Lançar e Executar Transações) na interface do Remix.

4. Na opção **Environment** (ambiente), selecione `Injected Web3`, e sob `Account`, selecione o seu endereço de Indexador.

5. Selecione o endereço de contrato do GraphToken, e cole este endereço (`0xc944E90C64B2c07662A292be6244BDf05Cda44a7`) próximo do botão `At Address`; clique neste botão para aplicar.

6. Chame a função `approve(spender, amount)` para aprovar o contrato de Staking. Preencha a lacuna `spender` com o endereço de contrato de Staking (`0xF55041E37E12cD407ad00CE2910B8269B01263b9`), e `amount` com os tokens a serem colocados (em wei).
...
3. Com o `Staking.abi` selecionado e aberto no editor, abra a seção `Deploy and Run Transactions` na interface do Remix.

4. Na opção **Environment** (ambiente), selecione `Injected Web3`, e sob `Account`, selecione o seu endereço de Indexador.

5. Selecione o endereço de contrato de Staking e cole este endereço (`0xF55041E37E12cD407ad00CE2910B8269B01263b9`) próximo do botão `At Address`; clique neste botão para aplicar.

6. Chame `stake()` para fazer stake de GRT no protocolo.

7. (Opcional) Os Indexadores podem aprovar outro endereço para operar sua infraestrutura de Indexador, a fim de poder separar as chaves que controlam os fundos daquelas que realizam ações rotineiras; por exemplo, alocar em subgraphs e servir queries (pagos). Para configurar o operador, chame `setOperator()` com o endereço do operador.
...
- **Fechada** - Um Indexador pode fechar uma alocação após a passagem de 1 epoch ([closeAllocation()](https://github.com/graphprotocol/contracts/blob/main/packages/contracts/contracts/staking/Staking.sol#L335), ou o seu agente indexador a fechará automaticamente após o **maxAllocationEpochs** (atualmente, 28 dias). Quando uma alocação é fechada com uma prova de indexação (POI) válida, as suas recompensas de indexação são distribuídas ao Indexador e aos seus Delegantes ([veja mais aqui](/indexing/overview/#how-are-indexing-rewards-distributed)).

É ideal que os Indexadores utilizem a função de sincronização off-chain para sincronizar lançamentos de subgraph à chainhead antes de criar a alocação on-chain. Esta ferramenta é especialmente útil para subgraphs que demorem mais de 28 epochs para sincronizar, ou que têm chances de falhar não-deterministicamente.
