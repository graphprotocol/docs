...
Para definir os campos a guardar nas suas entidades de subgraph, suba o arquivo `schema.graphql`. Por exemplo:
...
Com os objetos do Protobuf gerados, agora é possível manusear os dados do Substreams no seu arquivo `mappings.ts`, no diretório `./src`. Veja um exemplo de como extrair as transferências não derivadas associadas à ID de conta do Orca para entidades de subgraphs:
...
Este comando converte as definições do Protobuf em AssemblyScript, permitindo o uso destas no handler do subgraph.

## Conclusão

Parabéns! Está montado um subgraph movido a Substreams, baseado em gatilhos, para um token da SPL da Solana. Agora dá para personalizar mais o seu schema, seus mapeamentos, e os seus módulos de modo que combinem com o seu caso de uso específico.
