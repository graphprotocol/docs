...
Este manual requer o [yarn](https://yarnpkg.com/), assim como [as dependências necessárias para o desenvolvimento local da Substreams](https://substreams.streamingfast.io/developers-guide/installation-requirements).

## Como definir um pacote de Substreams

Um pacote de Substreams é composto de tipos(definido como [Protocol Buffers](https://protobuf.dev/)), módulos (escritos em Rust), e um arquivo `substreams.yaml`. que referencia os tipos e especifica como módulos são ativados. [Aprenda mais sobre o desenvolvimento de Substreams](/substreams/README).
...
Subgraphs movidos por Substreams introduzem um novo 'tipo' de fonte de dados, "substreams"; estes subgraphs só podem ter uma fonte de dados. Esta fonte de dados deve especificar a rede de Substreams, o pacote de Substreams ('spkg') como um local relativo de arquivos, e o módulo dentro daquele pacote de Substreams que produz mudanças de entidade compatíveis com o subgraph (neste caso 'map_entity_changes', do pacote de Substreams acima). O mapeamento é especificado, mas apenas identifica o tipo de mapeamento ("substreams/graph-entities") e a apiVersion.
