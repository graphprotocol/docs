...
Configurez avec succès un subgraph basé sur des déclencheurs et alimenté par des Substreams pour un jeton Solana SPL.
...
Définissez les champs que vous souhaitez enregistrer dans vos entités de subgraph en mettant à jour le fichier `schema.graphql`.
...
L'exemple ci-dessous montre comment extraire vers des entités de subgraph les transferts non dérivés associés à l'Id de compte Orca :
...
Félicitations ! Vous avez configuré avec succès un subgraph basé sur des déclencheurs et alimenté par des Substreams pour un jeton SPL Solana. Vous pouvez passer à l'étape suivante en personnalisant votre schéma, vos mappages et vos modules pour les adapter à votre cas d'utilisation spécifique.
