...
[GraphQL](https://graphql.org/learn/) est un langage d'interrogation pour les API et un moteur d'exécution pour ces requêtes avec vos données existantes. The Graph utilise GraphQL pour interroger les subgraphs.
...
Dans votre schéma de subgraph, vous définissez des types appelés `Entities`. Pour chaque type `Entity`, les champs `entity` et `entities` seront générés sur le type `Query` de haut niveau.
...
Les schémas GraphQL définissent généralement des types de base pour les `queries`, les `subscriptions` et les `mutations`. The Graph ne prend en charge que les `requêtes`. Le type de base `Query` pour votre subgraph est automatiquement généré à partir du schéma GraphQL qui est inclus dans votre [manifeste de subgraph](/developing/creating-a-subgraph/#components-of-a-subgraph).
...
Tous les subgraphs ont un objet `_Meta_` auto-généré, qui donne accès aux métadonnées du subgraph. Cet objet peut être interrogé comme suit :
...
`hasIndexingErrors` est un booléen indiquant si le subgraph a rencontré des erreurs d'indexation sur certains blocs précédents
