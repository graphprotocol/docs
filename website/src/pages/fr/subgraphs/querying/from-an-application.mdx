...
Après avoir déployé votre subgraph dans [Subgraph Studio](https://thegraph.com/docs/en/subgraphs/developing/deploying/using-subgraph-studio/), vous obtiendrez un endpoint qui ressemble à ceci :
...
> Cet endpoint est destiné à une utilisation active sur le réseau. Il vous permet d'utiliser diverses bibliothèques client GraphQL pour interroger le subgraph et alimenter votre application avec des données indexées.
...
Voyons comment récupérer les données d'un subgraph avec Apollo client:
...
Voyons comment récupérer les données d'un subgraph avec URQL :
