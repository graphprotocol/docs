...
3. Nous pouvons utiliser les [Chargeurs de champ dérivés](/subgraphs/developing/creating/graph-ts/api/#looking-up-derived-entities) pour débloquer la possibilité d'accéder directement et de manipuler les données à partir de relations virtuelles dans nos mappages de subgraphs.

## Conclusion

Utilisez la directive `@derivedFrom` dans les subgraphs pour gérer efficacement les tableaux à croissance dynamique, améliorant ainsi l'efficacité de l'indexation et la récupération des données.
