...
Oups, comme c'est malheureux, quand je déploie mon subgraph parfait dans [Subgraph Studio](https://thegraph.com/studio/), il échoue avec l'erreur _"Gravatar not found!"_.
...
L'utilisation du **forking de subgraph** permet d'éliminer cette étape. Voici à quoi cela ressemble :
...
2. Déploiement sur le Graph Node local, **_forking du subgraph défaillant_** et **_en commençant par le bloc problématique_**.
...
N'oubliez pas non plus de définir le champ `dataSources.source.startBlock` dans le manifeste du subgraph au numéro du bloc problématique, afin d'éviter d'indexer des blocs inutiles et de profiter du fork !

Voici donc ce que je fais :

1. Je démarre un Graph Node local ([voici comment faire](https://github.com/graphprotocol/graph-node#running-a-local-graph-node)) avec l'option `fork-base` fixée à : `https://api.thegraph.com/subgraphs/id/`, puisque je vais forker un subgraph, le subgraph bugué que j'ai déployé plus tôt, à partir de [Subgraph Studio](https://thegraph.com/studio/).
...
3. Après avoir fait les changements, je déploie mon subgraph sur le Graph Node local, **_en forkang le subgraph défaillant_** et en fixant `dataSources.source.startBlock` à `6190343` dans `subgraph.yaml` :
