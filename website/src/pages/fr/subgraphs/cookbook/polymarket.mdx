...
Interrogez les données onchain de Polymarket à l'aide de GraphQL via des subgraphs sur The Graph Network. Les subgraphs sont des API décentralisées alimentées par The Graph, un protocole d'indexation & d'interrogation des données des blockchains.

## Subgraph Polymarket sur Graph Explorer

Vous pouvez accéder à un environnement de requêtes interactif sur la [page du subgraph Polymarket sur The Graph Explorer](https://thegraph.com/explorer/subgraphs/Bx1W4S7kDVxs9gC3s2G6DS8kdNBJNVhMviCtin2DiBp?view=Query&chain=arbitrum-one), où vous pouvez tester n'importe quelle requête.
...
Pour plus d'informations sur l'interrogation des données de votre subgraph, lisez [ceci](/subgraphs/querying/introduction/).
