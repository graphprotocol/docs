...
Get fast answers to easily integrate and scale with The Graph's high-performance Token API. View the [OpenAPI document](https://token-api.thegraph.com/openapi) for endpoint formats and details.
...
During Beta, the Token API is has a generous allowance of free usage. There are no specific rate limits, but reasonable throttling exists to prevent abuse. High request volumes may trigger HTTP 429 errors. Monitor official announcements for future pricing changes after Beta.
