---
title: The Graph Client Tools
---

The Graph Client is a toolkit for querying Subgraphs using GraphQL. It supports browser and Node.js environments, and includes features like multi-indexer support, fetch strategies, client-side composition, and TypeScript typings.

## Background

The Graph's architecture is built on the principle of decentralized indexing, where multiple indexers process and serve blockchain data. However, this decentralized nature introduces unique challenges for dapp developers. The Client Tools emerged as a solution to these challenges, providing an abstraction layer that handles the complexities of interacting with a distributed network of Indexers.

Think of The Graph Client Tools as a smart routing system for your data requests. Just as a modern GPS system might route you through multiple paths to reach your destination based on traffic and road conditions, these tools intelligently manage your data requests across multiple indexers to ensure optimal performance and reliability.

## Core Concepts and Architecture

### Distributed Data Access

The fundamental principle behind the Client Tools is distributed data access. Unlike traditional APIs where you interact with a single endpoint, The Graph's ecosystem requires a more sophisticated approach. The tools handle multiple indexers simultaneously, implementing various strategies to ensure data reliability and availability.

### Fetch Strategies

Fetch strategies represent different approaches to handling data requests across multiple indexers. These strategies are analogous to different investment portfolio management techniques:

- The "fallback" strategy works like a backup plan, similar to having multiple insurance policies  
- The "race" strategy parallels the concept of market competition, where multiple providers compete to deliver the fastest service  
- The "highestValue" strategy resembles seeking the best offer among multiple vendors

### Client-Side Composition

Client-side composition is a powerful concept that allows developers to unify data from multiple subgraphs into a coherent whole. This is similar to how a database view combines data from multiple tables, but at the GraphQL API level. This approach enables developers to:

- Create unified data models across multiple data sources  
- Handle cross-chain data relationships  
- Build more maintainable and scalable applications

## Design Philosophy and Trade-offs

The Graph Client Tools are built around several key philosophical principles:

### Build-time vs Runtime

The tools emphasize build-time processing over runtime execution. This design decision mirrors the compilation vs interpretation trade-off in programming languages, where more work is done upfront to ensure better runtime performance.

### Flexibility vs Complexity

While the tools offer significant flexibility in data access patterns, this comes with increased complexity in configuration and setup. This trade-off is intentional, prioritizing power and control over initial simplicity.

### Decentralization Impact

The decentralized nature of The Graph introduces unique challenges that the Client Tools must address:

- Data consistency across multiple indexers  
- Network latency and reliability variations  
