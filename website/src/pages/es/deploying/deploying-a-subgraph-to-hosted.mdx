...
Ahora, supongamos que deseas implementar tu subgrafo en las redes `mainnet` y `sepolia`, y este es tu `subgraph.yaml`:
...
El comando `build` actualizará tu `subgraph.yaml` con la configuración de `sepolia` y luego volverá a compilar el subgrafo. Tu archivo `subgraph.yaml` ahora debería verse así:
...
Para ilustrar este enfoque, supongamos que un subgrafo debe deployarse en mainnet y Sepolia utilizando diferentes direcciones de contrato. Podrías entonces definir dos archivos de configuración proporcionando las direcciones para cada red:
...
Para deployar este subgrafo para mainnet o Sepolia ahora solo tendrías que ejecutar uno de los dos siguientes comandos:
