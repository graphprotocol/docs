...
Se não tiver certeza de qual escolher, é sempre bom usar a versão segura. Se o valor não existir, pode fazer uma declaração <code>if</code> precoce com um retorno no seu handler de subgraph.
