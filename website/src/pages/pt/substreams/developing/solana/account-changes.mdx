...
Para cada bloco de conta da Solana no Substreams, apenas a última atualização por conta é registrada; vide a [Referência do Protobuf](https://buf.build/streamingfast/firehose-solana/file/main:sf/solana/type/v1/account.proto). Se uma conta for excluída, será fornecida uma carga útil com `deleted == True`. Além disso, eventos de baixa importância são omitidos, tais como os que tem a conta de proprietário especial "Vote111111…", ou alterações que não afetam os dados da conta (ex: alterações no lamport).
