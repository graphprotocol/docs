...
Ao **implantar** um subgraph, você o envia para o [Subgraph Studio](https://thegraph.com/studio/), onde pode ser testado, preparado e revisado. A indexação de um subgraph implantado é realizada pelo [Upgrade Indexer](https://thegraph.com/blog/upgrade-indexer/), que é um único Indexador operado pela Edge & Node (que também é a proprietária do mesmo), em vez dos muitos indexadores descentralizados na The Graph Network. Um subgraph **implantado** é gratuito para uso, limitado por taxa, não visível ao público e destinado a fins de programação, preparação e teste.
...
Autentique e implante o seu subgraph. A chave de implantação ("deploy key") pode ser encontrada na página "Subgraph" do Subgraph Studio.
...
- Verificar os logs no painel de controlo para ver se há erros com o seu subgraph. Os logs de um subgraph operacional parecerão com isto:
...
Para editar o seu subgraph, clique o botão Publish ("Editar") no painel de controlo.
...
Para economizar gás, você pode selecionar o seu subgraph na mesma transação em que o edita através desta opção:
...
Agora, é possível solicitar queries do seu subgraph com o envio de queries GraphQL para o URL de Query do seu subgraph, visível ao clicar no botão Query.
