...
Você pode experimentar com os exemplos deste guia clonando o [repositório de Subgraph Demonstrativo](https://github.com/LimeChain/demo-subgraph)
...
- _Describes (descrições) não são obrigatórias. O test() ainda pode ser usado da maneira antiga, fora dos blocos describe()_
...
describe("handleUpdatedGravatar()", () => {
  ...
  describe("Quando não houver uma entidade", () => {
...
describe("Quando a entidade não existe", () => {
  test("ela deve criar um novo Gravatar com a id 0x1", () => {
    ...
  })
})

describe("Quando a entidade já existe", () => {
...
describe('handleUpdatedGravatars', () => {
  ...
  test('Atualiza o displayName', () => {
    ...
    // código que deve atualizar o displayName para 1o. Gravatar
  ...
  test('Atualiza o imageUrl', () => {
    ...
    // código que deve mudar imageUrl para https://www.gravatar.com/avatar/0x0
...
describe("handleUpdatedGravatar", () => {
  test("Atualiza o displayName", () => {
    ...
    // código que deve mudar o displayName para 1o. Gravatar
  ...
  test("Atualiza o imageUrl", () => {
    ...
    // código que deve mudar o imageUrl para https://www.gravatar.com/avatar/0x0
...
assert.notInStore('Gravatar', '0x124', 'Gravatar não deve estar armazenado')
...
assert.assertNotNull('not null', 'Deve não ser null')
...
test('Can call mappings with custom events', () => {
  ...
  // Chamar funções de mapeamento passando os eventos que acabamos de criar
  ...
  // Limpar o armazenamento para começar o próximo teste do zero
  clearStore()
})
...
![Matchstick diz "Todos os testes passados!”](/img/matchstick-tests-passed.png)
...
Os utilizadores podem encontrar falhas esperadas, com o flag shouldFail nas funções <code>test()</code>:
...
Caso o teste seja marcado com <code>shouldFail = true</code> mas NÃO falhe, isto será mostrado como um erro nos logs e o bloco de teste não terá êxito. E se for marcado com <code>shouldFail = false</code> (o estado normal), o executor de teste travará.
...
test('Exemplo simples de simulação de fonte de dados', () => {
...
Note que o <code>dataSourceMock.resetValues()</code> é chamado no final. Isto é porque os valores são lembrados quando mudados, e devem ser reconfigurados caso queira voltar aos valores padrão.
...
test('ethereum/contract dataSource creation example', () => {
  // Impor que não há dataSources criadas de modelo GraphTokenLockWallet
  assert.dataSourceCount('GraphTokenLockWallet', 0)

  // Criar uma nova datasource GraphTokenLockWallet com o endereço 0xA16081F360e3847006dB660bae1c6d1b2e17eC2A
  ...
  // Impor que foi criada uma dataSource com o endereço "0xA16081F360e3847006dB660bae1c6d1b2e17eC2B"
  // Lembrar que o tipo `Address` transforma para caixa baixa quando decodificado, então o endereço deve ser passado como caixa-baixa ao determinar se existe
...
export function handleMetadata(content: Bytes): void {
  ...
  // stringParam() será simulado no teste de handler
...
test('file/ipfs dataSource creation example', () => {
  ...
  // dataSource.stringParams usa o valor de dataSource.address(), então vamos simular o endereço
