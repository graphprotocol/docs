...
Запрашивайте данные Polymarket onchain с помощью GraphQL через субграфы в The Graph Network. Субграфы — это децентрализованные API, работающие на базе The Graph, протокола для индексирования и запроса данных из блокчейнов.

## Субграф Polymarket в Graph Explorer

Вы можете посетить интерактивную площадку для запросов на [странице субграфа Polymarket в The Graph Explorer](https://thegraph.com/explorer/subgraphs/Bx1W4S7kDVxs9gC3s2G6DS8kdNBJNVhMviCtin2DiBp?view=Query&chain=arbitrum-one), где Вы можете протестировать любой запрос.

![Polymarket Endpoint](/img/Polymarket-playground.png)

## Как пользоваться визуальным редактором запросов

Визуальный редактор запросов поможет Вам протестировать примеры запросов из Вашего субграфа.
...
Вы можете использовать этот ключ API в любом субграфе в [Graph Explorer](https://thegraph.com/explorer), его использование не ограничивается только Polymarket.
...
Дополнительную информацию о запросе данных из Вашего субграфа можно найти [здесь](/subgraphs/querying/introduction/).

Чтобы исследовать все способы оптимизации и настройки Вашего субграфа для улучшения производительности, читайте больше о [создании субграфа здесь](/developing/creating-a-subgraph/).
