---
title: TAP Geçiş Rehberi
---

The Graph’in yeni ödeme sistemi, **Zaman Çizelgesi Toplulaştırma Protokolü, TAP** hakkında bilgi edinin. Bu sistem, güven gereksinimini azaltıp hızlı ve etkili mikro işlemler sağlar.

## Genel Bakış

[TAP](https://docs.rs/tap_core/latest/tap_core/index.html), şu anda mevcut olan Scalar ödeme sisteminin yerine geçebilen bir sistemdir. Aşağıdaki ana özellikleri sağlar:
...
TAP, bir gönderenin alıcıya birden fazla ödeme yapmasını sağlar. **TAP Makbuzları (TAP Receipts),** bu ödemeleri tek bir ödeme olan **Makbuz Toplulaştırma Kuponu (Receipt Aggregate Voucher - RAV)** hâlinde toplulaştırır. Bu toplulaştırılmış ödeme daha sonra blokzinciri üzerinde doğrulanabilir, böylece işlem sayısı azalır ve ödeme süreci basitleşir.
...
Bir endeksleyici çalıştırmak için tipik gereksinimlere ek olarak, TAP güncellemelerini sorgulamak için bir `tap-escrow-subgraph` uç noktasına ihtiyacınız olacak. Bu uç noktaya erişmek için The Graph Ağı'na kullanabilir veya kendi `graph-node`'unuzda barındırabilirsiniz.
...
1. **Endeksleyici Aracı**
   ...
   - Yeni TAP kod yollarını etkinleştirmek ve TAP RAV'lerinin kullanılmasına izin vermek için yeni `--tap-subgraph-endpoint` argümanını kullanın.
