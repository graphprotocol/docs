...
- Be careful with handler computation limits, especially when using the `.pow()` function with large numbers.
  - As of [`graph-node v0.26.0`](https://github.com/graphprotocol/graph-node/releases/tag/v0.26.0) gas metering was introduced.
- Required fields are handled differently depending on the `graph-cli` version used.
  - In older `graph-cli` releases default values are automatically set, but this was changed to require non-nullable fields to be set in [`graph-cli vTODO?`]().
