...
The logs should tell you if there are any errors. If your subgraph is failing, you can query the subgraph health by using the [GraphiQL Playground](https://graphiql-online.com/). Use this endpoint: `https://api.thegraph.com/index-node/graphql`.
Note that you can leverage the query below and input your deployment ID for your subgraph. The deployment ID (`Qm...`)  can be located on the Subgraph page under **Details**. If your subgraph has failed, the query below will give you an error message so you can debug accordingly.
...
You can now query your subgraph by following [these instructions](/querying-subgraphs/query-the-graph). You can query from your dapp if you don't have your API key via the free, rate limited temporary query URL that can be used for development and staging. You can read the additional instructions for how to query a subgraph from a frontend application [here](/cookbook/querying-from-your-app).
...
The logs should tell you if there are any errors. If your subgraph is failing, you can query the subgraph health by using the [GraphiQL Playground](https://graphiql-online.com/). Use this endpoint: `https://api.thegraph.com/index-node/graphql`.
Note that you can leverage the query below and input your deployment ID for your subgraph. The deployment ID (`Qm...`)  can be found on the Subgraph page. If your subgraph has failed, the query below will give you an error message so you can debug accordingly.
