---
title: Grafting on Existing Subgraphs
---
...
In this tutorial, we will be covering a basic usecase. We will replace an existing contract with an identical contract (with a new address). Then, graft the existing subgraph onto the new contract.

## Building an Existing Subgraph

To be able to build and deploy the existing subgraph used in this tutorial, you need to follow the instructions found here:

1. `@graphprotocol/graph-cli`- This is a command-line tool for building and deploying subgraphs. [Click here](https://www.npmjs.com/package/@graphprotocol/graph-cli) to download using `npm`.

Once these are installed, you can start building the subgraph with the instructions found here:

- [Subgraph example repo](https://github.com/schmidsi/hackathon-starterkit#quick-start)

> Note: The contract was taken from the following [Hackathon Starterkit](https://github.com/schmidsi/hackathon-starterkit).
...
Defines the data sources of interest, and how they should be processed. The Lock contract is our data source. We will be focusing on this file only in this tutorial.
...
The requirements for subgraphs are covered by the [existing documentation](/developing/creating-a-subgraph/#the-graphql-schema).
...
- The Locks' data source is the abi and contract address we will get when we compile and deploy the contract
...
Once you have verified the subgraph is indexing properly, you can deploy the subgraph with grafting.
...
2. Add your `base` subgraph ID to the `graft` section in the `subgraph.yaml` file. This is the `Deployment ID` of your original `graph-example` subgraph. You can find this in the Graph Studio UI.
...
You can see that the `graft-replacement` subgraph is indexing from older `graph-example` data and newer data from the new contract address. The block number in `graft-replacement` specified in the `graft` section of the `subgraph.yaml` file has the last `Withdrawal` data from the old contract.
