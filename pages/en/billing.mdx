...
Invoices are statements of payment amounts owed by a customer and are typically generated on a weekly basis in the system. You'll be required to pay fees based on the query fees you generate using your API keys. While The Graph protocol operates on Ethereum Mainnet, the billing contract lives on the [Arbitrum](https://arbitrum.io/) network to reduce transaction times and cost. The billing contract will allow you to:
