...
### When will the decentralized network support my preferred chain?

There is no set timeline per chain, they will be dictated by Indexer readiness via the [MIPs program](https://thegraph.com/migration-incentive-program/) where new chains are tested by Indexers. As new chains are supported on the network, users will receive ample notification to prepare for migration. Core devs and contributors to The Graph ecosystem are working to implement support for more chains as soon as possible.
