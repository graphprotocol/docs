...
Using **subgraph forking** we can essentially eliminate this step. Here is how it looks:
  ...
  2. Deploy to the local Graph node, ***forking the failing subgraph*** and ***starting from the problematic block***.
...
And I answer:
  ...
  1) `fork-base` is the "base" URL, such that when the *subgraph id* is appended the resulting URL (`<fork-base>/<subgraph-id>`) is a valid GraphQL endpoint for the subgraph's store. 2) Forking is easy, no need to sweat:
  ```bash
