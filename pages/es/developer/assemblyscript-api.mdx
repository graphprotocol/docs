...
Los subgrafos creados con [`graph init`](/developer/create-subgraph-hosted) vienen configurados previamente. Todo lo necesario para instalar estas dependencias es correr uno de los siguientes comandos:
...
|  0.0.5  | AssemblyScript actualizado a la version 0.19.10 (que incluye ultimos cambios, mira [`Migration Guide`](/developer/assemblyscript-migration-guide))<br />`ethereum.transaction.gasUsed` renombrado a `ethereum.transaction.gasLimit` |
...
- `concat(other: ByteArray) : ByteArray` - regresa un nuevo `ByteArray` consistente en `this` directamente seguido por `other`
- `concatI32(other: i32) : ByteArray` - regresa un nuevo `ByteArray` consistente en `this` directamente seguido por ela representacion de byte de `other`
...
- `fromI32(i: i32) : Bytes` - Convierte `i` en un array de bytes

_Tipo de conversiones_

- `b.toHex()` – returna un string hexadecimal que representa los bytes en el array
- `b.toString()` – convierte los bytes en los array en un string de caracteres unicode
...
- `b.concatI32(other: i32) : ByteArray` - retorna un nuevo `Bytes` consistente en `this` seguir directamente la representacion del byte `other`
...
- `Address.fromString(s: string): Address` – crea una `Address` desde un string hexadecimal
- `Address.fromBytes(b: Bytes): Address` – crea un `Address` desde `b` que debe tener exactamente 20 bytes de longitud. Pasar un valor con menos o mas bytes dara lugar a un error
...
La actualización de las propiedades del array es un poco más complicada, ya que al obtener un array de una entidad se crea una copia de ese array. Esto significa que las propiedades del array tienen que ser establecidas de nuevo explícitamente después de cambiar el array. Lo siguiente supone que la `entity` tiene un `numbers: [BigInt!]!` campo.
...
Con las clases generadas, las conversiones entre los tipos de Ethereum y los tipos [incorporados](#built-in-types) tienen lugar detras de escena para que los autores de los subgrafos no tengan que preocuparse por ellos.
...
y un evento de firma `Transfer(address,address,uint256)` en Ethereum, el `from`, `to` y `amount` valores del tipo `address`, `address` y `uint256` se convierten `Address` y `BigInt`, lo que permite pasarlos a las propiedades `Bytes!` y `BigInt!` de la `Transfer` entidad:
...
La API `log` permite a los subgrafos registrar información en la salida estándar del Nodo Graph así como en Graph Explorer. Los mensajes pueden ser registrados usando diferentes niveles de registro. Se proporciona una sintaxis de string de formato básica para componer los mensajes de registro a partir de un argumento.
...
La API `log` toma un string de formato y un array de valores de string. A continuación, sustituye los marcadores de posición por los valores de string del array. El primer marcador de posición `{}` se sustituye por el primer valor del array, el segundo marcador de posición `{}` se sustituye por el segundo valor y así sucesivamente.
...
En el ejemplo siguiente, sólo se registra el primer valor del array de argumentos, a pesar de que el array contiene tres valores.
...
**Nota:** `ipfs.cat` no es determinista por el momento. Si el archivo no puede ser recuperado a través de la red IPFS antes de que la solicitud se agote, devolverá `null`. Debido a esto, siempre vale la pena comprobar el resultado para `null`. Para asegurarse de que los archivos pueden ser recuperados, tienen que estar anclados al nodo IPFS al que se conecta Graph Node. En el [servicio alojado](https://thegraph.com/hosted-service), esto es [https://api.thegraph.com/ipfs/](https://api.thegraph.com/ipfs). Consulta la sección [IPFS pinning](/developer/create-subgraph-hosted#ipfs-pinning) para obtener más información.
...
- `json.fromBytes(data: Bytes): JSONValue` - analiza los datos JSON a partir de un array `Bytes` interpretado como una secuencia UTF-8 válida
...
La clase `JSONValue` proporciona una forma de extraer valores de un documento JSON arbitrario. Como los valores JSON pueden ser booleans, números, arrays y más, `JSONValue` viene con una propiedad `kind` para comprobar el tipo de un valor:
...
Cuando el tipo de un valor es cierto, se puede convertir a un [tipo incorporado](#built-in-types) utilizando uno de los siguientes métodos:
...
| String               | Adress               | Address.fromString(s)        |
