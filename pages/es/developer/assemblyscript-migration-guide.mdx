...
```typescript
ERROR TS2322: Type '~lib/string/String | null' is not assignable to type '~lib/string/String'. class Container {
  data: string | null
}
...
let somethingOrElse: string = data ? data : 'else' // compiles just fine :) data : 'else' // compiles just fine :) data : 'else' // compiles just fine :)
```
