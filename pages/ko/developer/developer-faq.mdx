...
현재 dapp에 권장되는 접근 방식은 프론트엔드에 키를 추가하고 최종 사용자에게 노출하는 것입니다. 즉, 해당 키를 _yourdapp.io_및 subgraph와 같은 호스트 이름으로 제한할 수 있습니다. 게이트웨이는 현재 Edge & Node에 의해 실행 중입니다. 게이트웨이의 책임 중 일부는 남용 행위를 모니터링하고 악의적인 클라이언트의 트래픽을 차단하는 것입니다.
