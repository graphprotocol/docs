...
| `BigInt`     | 큰 정수. 이더리움의 code>uint32</code>, `int64`, `uint64`, ..., `uint256` 유형에 사용됩니다. 참고: , such as `int32`, `uint24` 또는 `int8`과 같은 `uint32` 아래의 모든 항목은 `i32`로 표시됩니다. |
...
`User` 엔터티 유형에서 `Organization` 엔터티 유형으로 reverse lookup를 정의합니다. 아래 예에서는 `Organization` 엔터티 내에서 `members` 속성을 조회하여 이를 수행합니다. 쿼리에서 `User` 의 `organizations` 필드는 사용자의 ID를 포함하는 모든 </code>`organizations` 엔터티를 찾아서 해결됩니다.
...
그런 다음 매니페스트에 _data source templates_을 추가합니다. 이는 `source` 아래에 미리 정의된 컨트랙트 주소가 없다는 점을 제외하고 일반 데이터 소스와 동일합니다. 일반적으로 상위 컨트랙트에서 관리하거나 참조하는 각 하위 컨트랙트 유형에 대해 하나의 템플릿을 정의합니다.
...
서브그래프가 처음 배치되면 해당 체인의 제네시스 블록(또는 각 데이터 소스로 정의된 `startBlock`)에서 이벤트 인덱싱을 시작합니다. 어떤 상황에서는 기존 서브그래프의 데이터를 재사용하고 인덱싱을 시작하는 것이 유리합니다. 훨씬 나중에 블록. 이 인덱싱 모드를 _Grafting_이라고 합니다. 예를 들어, 그래프팅은 매핑의 간단한 오류를 빠르게 극복하거나 실패한 후 기존 하위 그래프가 일시적으로 다시 작동하도록 개발하는 동안 유용합니다.
