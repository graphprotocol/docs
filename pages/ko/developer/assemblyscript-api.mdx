...
[`graph init`](/developer/create-subgraph-hosted)로 생성된 서브그래프는 미리 구성된 의존성들을 함께 동반합니다. 이러한 의존성들을 설치하려면 다음 명령 중 하나를 실행해야 합니다.
...
- 이더리움 스마트 컨트렉트, 이벤트, 블록, 트랜젝션, 그리고 이더리움 벨류들과 작업하기 위한 `ethereum` API
...
```typescript
'@graphprotocol/graph-ts'로 부터 { ByteArray }를 입력합니다.
```
...
```typescript
'@graphprotocol/graph-ts'로 부터 { BigInt }를 입력합니다.
```
...
- `BigInt.fromUnsignedBytes(x: Bytes): BigInt` – `bytes`를 부호 없는 little-endian 정수로 해석합니다. If your input is big-endian, call `.reverse()` first.
