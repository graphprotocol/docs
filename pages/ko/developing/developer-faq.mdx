...
여러 네트워크에 대해 별도의 이름이 필요합니다. 같은 이름 아래 다른 서브그래프를 가질 수는 없지만 여러 네트워크에 대해 단일 코드베이스를 사용하는 편리한 방법이 있습니다. 이에 대한 자세한 내용은 저희의 설명서를 참고하시기 바랍니다: [Redeploying a Subgraph](/deploying/deploy-subgraph-hosted#redeploying-a-subgraph)
...
[데이터 소스 템플릿](/developing/create-subgraph-hosted#data-source-templates)에서 "데이터 소스 템플릿 인스턴스화" 섹션을 확인하십시오.
...
예, 컨트랙트가 배포된 블록에서 인덱싱을 시작하려면 선택적 시작 블록 기능을 살펴봐야 합니다: [Start blocks](/developing/create-subgraph-hosted#start-blocks)
