...
전체 텍스트 검색 쿼리 필드는 서브그래프 스키마에 추가하고 사용자 정의할 수 있는 표현식 텍스트 검색 API를 제공합니다. 서브그래프에 전체 텍스트 검색을 추가하려면 [Defining Fulltext Search Fields](/developing/create-subgraph-hosted#defining-fulltext-search-fields)를 참조하시길 바랍니다.
