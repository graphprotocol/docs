...
아직 확인하지 않았다면 [서브그래프 매니페스트](/developing/create-subgraph-hosted#the-subgraph-manifest)를 구성하는 파일을 작성하는 방법과 [Graph CLI](https://github.com/graphprotocol/graph-cli)를 설치하여 서브그래프에 대한 코드를 생성하는 방법을 확인하십시오. 이제 서브그래프를 Hosted Service라고도 하는 호스팅 서비스에 배포할 시간입니다.
...
새 서브그래프를 저장하면 그래프 CLI를 설치하는 방법, 새 서브그래프에 대한 스캐폴딩을 생성하는 방법 및 서브그래프를 배포하는 방법에 대한 도움말이 포함된 화면이 표시됩니다. 처음 두 단계는 [서브그래프 정의 섹션](/developing/define-subgraph-hosted)에서 다루었습니다.
