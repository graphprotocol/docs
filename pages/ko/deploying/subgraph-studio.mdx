...
서브그래프를 생성한 후 [CLI](https://github.com/graphprotocol/graph-cli) 또는 명령줄 인터페이스를 사용하여 배포할 수 있습니다. CLI를 사용하여 서브그래프를 배포하면 플레이그라운드를 사용하여 서브그래프를 테스트할 수 있는 Studio로 서브그래프가 푸시됩니다. 이렇게 하면 결국 그래프 네트워크에 게시할 수 있습니다. CLI 설정에 대한 자세한 내용은 [여기를 확인하시기 바랍니다](/developing/define-subgraph-hosted#install-the-graph-cli)(pst, 배포 키가 있는지 확인하십시오). 배포는 게시와 **다릅니다**. 서브그래프를 deploy 할 때 여러분들은 단순하게 테스트할 수 있는 Studio로 푸시하는 것입니다. 반대로 서브그래프를 배포(publish)하면 온체인에 게시하는 것입니다.
