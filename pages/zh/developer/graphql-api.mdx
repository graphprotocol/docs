...
查询从集合开头偏移 10 个位置的 10个`Token` 实体：
...
如果客户端需要检索大量实体，则基于属性进行查询和过滤会明显提高性能。 例如，客户端可以使用以下查询检索大量token：
...
请注意，当前的实现仍然受到某些可能违反这些保证的限制。 当前的实现不能总是确认给定的区块哈希是否在主链上，或者由于受到同时运行的区块重组的影响，通过区块哈希查询区块的结果是否是确定性的。当区块是最终区块并且确认已在主链上时，通过区块哈希查询的结果就不会受到影响。 [这个issue](https://github.com/graphprotocol/graph-node/issues/1405)详细解释了这些限制。
...
使用 `or` 运算符，此查询将过滤到在其全文字段中具有“anarchism”或“crumpet”变体的blog实体。
...
`follow by` 运算符指定全文文档中相隔特定距离的单词。 以下查询将返回所有“decentralize”后跟着“philosophy”单词的blog
...
可以将全文运算符进行组合来制作更复杂的过滤器。 在下面的示例中，使用前缀搜索运算符与其他运算符的结合，可以实现匹配所有以“lou”开头后，跟着“music”单词的blog实体。
...
GraphQL 模式通常定义`查询`、`订阅`和`变种`的根类型。 The Graph目前仅支持`查询`。 子图的根`查询`类型是按照包含在子图清单中的 GraphQL 模式定义自动生成的。
