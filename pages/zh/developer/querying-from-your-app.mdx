...
Since some fields might be computed or proxied on the server-side, fetching fewer fields can make a big difference!
...
`numActiveTranscoders` might be slower (computed) field to resolve.

If not needed, removing it will improve the overall query performance (*especially in a list context:* `list: 5`).
