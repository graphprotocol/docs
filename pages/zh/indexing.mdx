...
索引人基础设施的中心是Graph节点，它监控以太坊，根据子图定义提取和加载数据，并将其作为[GraphQL API](/about/introduction#how-the-graph-works)提供。 The Graph节点需要连接到以太坊EVM节点端点，以及用于获取数据的IPFS节点；一个用于存储的PostgreSQL数据库；以及促进其与网络互动的索引人组件。
