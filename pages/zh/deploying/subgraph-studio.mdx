...
创建子图后，您将能够使用 [CLI](https://github.com/graphprotocol/graph-cli) 或命令行界面来部署它。 使用 CLI 部署子图会将子图推送到 Studio，您可以在其中使用 Playground 测试子图。 这最终将允许您发布到 Graph Network。 有关 CLI 设置的更多信息，[检查一下](/developing/define-subgraph-hosted#install-the-graph-cli)（pst，确保您手头有部署密钥）. 请记住，部署是**与**发布不同。 当您部署一个子图时，您只需将其推送到您可以对其进行测试的 Studio。 相反，当你发布一个子图时，你是在链上发布它。
