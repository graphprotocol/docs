...
- 使用 [全文搜索字段](/developing/creating-a-subgraph/#defining-fulltext-search-fields)。
- 以太坊主网以外的索引链。
...
2. 在 [Subgraph Studio](https://thegraph.com/studio/) 上创建子图。 在 [Subgraph Studio 文档](/deploying/subgraph-studio) 和 [这个视频教程](https://www.youtube.com/watch? v=HfDgC2oNnwo)。
...
升级需要将 GRT 从旧版本的子图迁移到新版本。 这意味着每次升级都会创建一条新的结合曲线（更多关于结合曲线的信息[这里](/network/curating#bonding-curve-101)）。
...
按照[此处](/managing/deprecating-a-subgraph)的步骤弃用您的子图并将其从 The Graph Network 中删除。

## 在 Graph 网络上查询子图 + 计费

托管服务的设置允许开发人员不受任何限制地部署他们的子图。

为了让 The Graph Network 真正去中心化，必须支付查询费用作为协议激励的核心部分。 有关订阅 API 和支付查询费用的更多信息，请查看[此处](/querying/billing)的计费文档。
