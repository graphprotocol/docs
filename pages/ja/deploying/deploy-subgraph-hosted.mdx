...
まだチェックアウトしていない方は、サブグラフの[subgraph manifest](/developing/create-subgraph-hosted#the-subgraph-manifest)を構成するファイルの書き方と、サブグラフのコードを生成するための[Graph CLI](https://github.com/graphprotocol/graph-cli)のインストール方法を確認してください。さて、いよいよサブグラフをホステッドサービスにデプロイします。
...
新しいサブグラフを保存すると、Graph CLI のインストール方法、新しいサブグラフの足場の生成方法、サブグラフのデプロイ方法についてのヘルプ画面が表示されます。最初の 2 つのステップは、「[Define a Subgraph section](/developing/define-subgraph-hosted)」で説明しました。
