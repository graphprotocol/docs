...
インデキシング報酬は、年間 3％の発行量に設定されているプロトコル・インフレから得られます。 報酬は、それぞれのサブグラフにおけるすべてのキュレーション・シグナルの割合に基づいてサブグラフに分配され、そのサブグラフに割り当てられたステークに基づいてインデクサーに分配されます。 **報酬の獲得には、アービトレーション・チャーターで定められた基準を満たす有効なインデックス証明（POI）を使用して割り当てを終了する必要があります。 **
...
`クエリフィーカット` と`インデキシングリワードカット` の値は、インデクサーが クールダウンブロックと共に設定できるデリゲーションパラメータで、インデクサーとそのデリゲーター間の GRT の分配を制御するためのものです。 デリゲーションパラメータの設定方法については、[Staking in the Protocol](/indexing#stake-in-the-protocol)の最後のステップを参照してください。
...
[Google Cloud で Terraform を使ってサーバーインフラを構築するのセクション ](/indexing#setup-server-infrastructure-using-terraform-on-google-cloud) を参照してください。
...
インデクサーがプロトコルに GRT をステークすると、[indexer components](/indexing#indexer-components)を起動し、ネットワークとのやりとりを始めることができます。
