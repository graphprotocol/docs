...
`BigInt`は大きな整数を表すのに使われます。 これには、Ethereumの`uint32`～`uint256` 、`int64` ～`int256`の値が含まれます。 Everything below `uint32`, such as `int32`, `uint24` or `int8` is represented as `i32`.
