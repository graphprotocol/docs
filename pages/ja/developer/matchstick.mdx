...
You can also use Matchstick on **WSL** (Windows Subsystem for Linux), but you need to have a few extra things set up - make sure you're using the latest version of Node.js and that you have **libpq** installed. Also, with WSL you should add a test script to your `package.json` file, which can be as simple as `"test": "graph test"` and use that instead of the usual `graph test` command. So depending on your preferred package manager you'd either run `npm run test` or `yarn test`. The difference is that that way you'll be using the project-level graph-cli, and not your global one, which is still having issues on WSL.
...
❗ If you encounter issues with using the Docker approach and you get an error message complaining that it "can't find the Matchstick version", please follow the [manual Docker setup](https://github.com/LimeChain/matchstick#docker-) and then edit the Dockerfile on the line where you have the `<MATCHSTICK_VERSION>` variable. Change `<MATCHSTICK_VERSION>` to the newest release tag (with a green "Latest" label) available in the [Releases page](https://github.com/LimeChain/matchstick/releases) on GitHub, for instance `0.4.4`.
