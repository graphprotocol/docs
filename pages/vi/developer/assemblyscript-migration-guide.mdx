...
let bytes = new Bytes(2) < ByteArray > bytes // ngắt trong thời gian chạy :(
```
...
Đây không phải là thay đổi AssemblyScript trực tiếp, nhưng bạn có thể phải cập nhật tệp `schema.graphql` của mình.

Giờ đây, bạn không còn có thể xác định các trường thuộc loại Non-Nullable Lists của mình nữa. Nếu bạn có một lược đồ như thế này:
...
Bạn sẽ phải thêm `!` vào thành viên của loại List, như sau:
...
Điều này đã thay đổi do sự khác biệt về tính nullability giữa các phiên bản AssemblyScript và nó liên quan đến tệp `src/generated/schema.ts` (đường dẫn mặc định, bạn có thể đã thay đổi điều này).
