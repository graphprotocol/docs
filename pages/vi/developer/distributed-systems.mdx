...
```javascript
/// Cập nhật biến protocol.paused lên phiên bản mới nhất
/// giá trị đã biết trong một vòng lặp bằng cách tìm nạp nó bằng The Graph.
async function updateProtocolPaused() {
  // Có thể bắt đầu với minBlock ở mức 0. Truy vấn sẽ được phục vụ
  // bằng cách sử dụng khối mới nhất có sẵn. Đặt minBlock thành 0 cũng
  // giống như việc loại bỏ lập luận đó.
  let minBlock = 0

  for (;;) {
    // Lên lịch một lời hứa sẽ sẵn sàng một lần
    // khối Ethereum tiếp theo có thể sẽ có sẵn.
    ...
    // VIỆC CẦN LÀM: Làm điều gì đó với dữ liệu phản hồi tại đây thay vì ghi lại dữ liệu đó.
    console.log(response.protocol.paused)

    // Ngủ để đợi khối tiếp theo
    await nextBlock
  }
}
```
...
```javascript
/// Nhận danh sách tên miền từ một khối duy nhất bằng cách sử dụng phân trang
async function getDomainNames() {
  // Đặt giới hạn về số lượng mặt hàng tối đa để kéo.
  let pages = 5
  const perPage = 1000

  // Truy vấn đầu tiên sẽ nhận được trang kết quả đầu tiên và cũng nhận được khối
  // băm để phần còn lại của các truy vấn nhất quán với phần đầu tiên.
  ...
  let query
  // Tiếp tục tìm nạp các trang bổ sung cho đến khi chúng ta đạt đến giới hạn
  // Tổng cộng 5 trang (đã nêu ở trên) hoặc chúng ta biết rằng chúng ta đã đến trang cuối cùng
  // bởi vì trang có ít thực thể hơn một trang đầy đủ.
