...
> Ghi chú: nếu bạn đã tạo một subgraph trước đến `graph-cli`/`graph-ts`phiên bản`0.22.0`, bạn đang sử dụng phiên bản AssemblyScript cũ hơn, chúng tôi khuyên bạn nên xem qua [`Hướng dẫn Di chuyển`](/release-notes/assemblyscript-migration-guide)
...
Subgraphs được tạo với [`graph init`](/developing/create-subgraph-hosted) đi kèm với các phụ thuộc được định cấu hình trước. Tất cả những gì cần thiết để cài đặt các phần phụ thuộc này là chạy một trong các lệnh sau:
...
| 0.0.5 | AssemblyScript đã nâng cấp lên phiên bản 0.19.10 (điều này bao gồm các thay đổi vi phạm, vui lòng xem [`Hướng dẫn Di chuyển`](/release-notes/assemblyscript-migration-guide))<br />`ethereum.transaction.gasUsed` đổi tên thành `ethereum.transaction.gasLimit` |
