...
يجب أن يكون لكل كيان ID فريد لتجنب التضارب مع الكيانات الأخرى. من الشائع إلى حد ما أن تتضمن بارامترات الأحداث معرفا فريدا يمكن استخدامه. Note: Using the transaction hash as the ID assumes that no other events in the same transaction create entities with this hash as the ID.
