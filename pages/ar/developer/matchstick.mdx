...
علينا أولا إنشاء ملف اختبار في مشروعنا. لقد اخترنا الاسم `gravity.test.ts`. في الملف الذي تم إنشاؤه حديثا ، نحتاج إلى تعريف دالة باسم `()runTests`. من المهم أن يكون للدالة هذا الاسم بالضبط. هذا مثال على الشكل الذي قد يبدو عليه اختبارنا:
...
هذا كثير لفك الحزمة! أولاً ، من المهم ملاحظة أننا نستورد أشياء من `matchstick-as` ، ومكتبتنا المساعدة AssemblyScript (موزعة كوحدة npm). يمكنك العثور على repository [ هنا ](https://github.com/LimeChain/matchstick-as). `matchstick-as` يوفر لنا methods اختبار مفيدة ويعرف أيضا دالة`()test` التي سنستخدمها لبناء كتل الاختبار الخاصة بنا. الباقي واضح ومباشر - إليك ما يحدث:
...
- وأخيرا - نقوم بتنظيف المخزن باستخدام `() clearStore` بحيث يمكن أن يبدأ اختبارنا التالي بكائن مخزن جديد وفارغ. يمكننا تعريف العديد من كتل الاختبار كما نريد.
...
يمكنك تصدير دالة غلاف الاختبارات في ملف الـ mappings كما يلي:
...
الآن لإجراء اختبارنا، تحتاج ببساطة إلى تشغيل التالي في مجلد جذر الـ subgraph:
...
### استدعاء جميع الـ mappings مع تركيبات الأحداث

يمكن للمستخدمين استدعاء الـ mappings مع تركيبات الاختبار.
...
### التفاعل مع البيانات الوصفية للحدث
...
يمكن للمستخدمين تأكيد عدم وجود كيان في المخزن. الدالة تأخذ نوع كيان وid. إذا كان الكيان موجودا بالفعل في المخزن، فسيفشل الاختبار مع ظهور رسالة خطأ. فيما يلي مثال سريع على كيفية استخدام هذه الدالة:
