...
يجب أن يكون لكل كيان حقل `id` ، وهو من النوع `ID!` (string). حقل `id` يتم تقديمه كمفتاح رئيسي ويجب أن يكون فريدا في كل الكيانات لنفس النوع.
...
| `Int`        | المواصفات الفنية لـ GraphQL تعرف `Int` بحجم 32 بايت.                                                                                                                            |
...
| `BigDecimal` | `BigDecimal` يتم تمثيل الكسور العشرية عالية الدقة على أنها signficand وأسس.. يتراوح نطاق الأس من −6143 إلى +6144. مقربة إلى 34 رقما.                                            |
...
يتم تعريف العلاقات للكيانات تماما مثل أي حقل آخر عدا أن النوع المحدد هو كيان آخر.
...
يمكن تعريف البحث العكسي لكيان من خلال حقل `derivedFrom`. ويؤدي هذا إلى إنشاء حقل افتراضي للكيان الذي قد يتم الاستعلام عنه ولكن لا يمكن تعيينه يدويا من خلال الـ mappings API. بالأحرى، هو مشتق من العلاقة المعرفة للكيان الآخر. وبالنسبة لمثل هذه العلاقات ، نادرا ما يكون من المنطقي تخزين جانبي العلاقة ، وسيكون أداء الفهرسة والاستعلام أفضل عندما يتم تخزين جانب واحد فقط ويتم اشتقاق الجانب الآخر.
...
هذه الطريقة الأكثر إتقانا لتخزين علاقات متعدد_لمتعدد والتي ستؤدي إلى تخزين بيانات أقل للـ subgraph، وبالتالي غالبا إلى subgraph ما يكون أسرع في الفهرسة والاستعلام.

#### إضافة تعليقات إلى المخطط (schema)

وفقا لمواصفات GraphQL الفنية ، يمكن إضافة التعليقات فوق خاصيات كيان المخطط باستخدام الاقتباسات المزدوجة `""`. هذا موضح في المثال أدناه:
...
استعلامات بحث النص الكامل تقوم بفلترة وترتيب الكيانات بناء على إدخال نص البحث. استعلامات النص الكامل قادرة على إرجاع التطابقات للكلمات المتشابهة عن طريق معالجة إدخال نص الاستعلام إلى جذر الكلمة (stems) قبل مقارنة ببيانات النص المفهرس.

تعريف استعلام النص الكامل يتضمن اسم الاستعلام وقاموس اللغة المستخدمة لمعالجة حقول النص وخوارزمية الترتيب المستخدمة لترتيب النتائج والحقول المضمنة في البحث. كل استعلام نص كامل قد يمتد إلى عدة حقول ، ولكن يجب أن تكون جميع الحقول المضمنة من نوع كيان واحد.
...
يمكن استخدام حقل المثال `bandSearch` في الاستعلامات وذلك لفلترة كيانات `Band` استنادا إلى المستندات النصية في الـ `name` ، `description` و `bio`. انتقل إلى [GraphQL API - Queries](/developer/graphql-api#queries) للحصول على وصف لـ API الخاص ببحث النص الكامل ولمزيد من الأمثلة المستخدمة.
...
تعمل الـ mappings على تحويل بيانات الإيثيريوم التي تصدرها الـ mappings الخاصة بك إلى كيانات محددة في مخططك. تتم كتابة الـ mappings في مجموعة فرعية من [ TypeScript ](https://www.typescriptlang.org/docs/handbook/typescript-in-5-minutes.html) تسمى [AssemblyScript](https://github.com/AssemblyScript/assemblyscript/wiki) والتي يمكن ترجمتها إلى ([WebAssembly](https://webassembly.org/)). ويعتبر AssemblyScript أكثر صرامة من TypeScript العادي ، ولكنه يوفر تركيبا مألوفا للجمل.
...
يأخذ المعالج الأول حدث `NewGravatar` وينشئ كيان `Gravatar` جديد بـ `new Gravatar (event.params.id.toHex ())` ،مالئا حقول الكيان باستخدام بارامترات الحدث المقابلة. ويتم تمثيل instance الكيان بالمتغير `gravatar` ، مع قيمة معرف `()event.params.id.toHex`.
...
نحن نقدم [Graph Typescript مكتبة ](https://github.com/graphprotocol/graph-ts) والتي تحتوي على أدوات مساعدة للتفاعل مع مخزن Graph Node حيث أنها ملائمة لمعالجة بيانات العقد الذكي والكيانات. ويمكنك استخدام هذه المكتبة في mappings الخاص بك عن طريق استيراد `graphprotocol/graph-ts` in `mapping.ts@`.
...
بالإضافة إلى ذلك ، يتم إنشاء فئة واحدة لكل نوع كيان في مخطط الـ subgraph's GraphQL. وتوفر هذه الفئات إمكانية تحميل كيان نوع-آمن (type-safe) والقراءة والكتابة إلى حقول الكيان بالإضافة إلى `save()` method لكتابة الكيانات للمخزن. تمت كتابة جميع فئات الكيانات إلى `<OUTPUT_DIR>/schema.ts`, مما يسمح للـ mappings باستيرادها باستخدام
...
النمط الشائع في عقود Ethereum الذكية هو استخدام عقود السجل أو المصنع (registry or factory)، حيث أن أحد العقود ينشئ أو يدير أو يشير إلى عدد اعتباطي من العقود الأخرى التي لكل منها حالتها وأحداثها الخاصة. وعناوين هذه العقود الفرعية قد تكون أو لا تكون معروفة مقدما وقد يتم إنشاء و / أو إضافة العديد من هذه العقود بمرور الوقت. وهذا هو السبب في أنه في مثل هذه الحالات ، يكون تعريف مصدر بيانات واحد أو عدد ثابت من مصادر البيانات أمرا مستحيلا ويلزم اتباع نهج أكثر ديناميكية: _قوالب مصدر البيانات_.
...
في الخطوة الأخيرة ، تقوم بتحديث الـ mapping لعقدك الرئيسي وذلك لإنشاء instance لمصدر بيانات ديناميكي من أحد القوالب. في هذا المثال ، يمكنك تغيير mapping العقد الرئيسي لاستيراد قالب `Exchange` واستدعاء method الـ`Exchange.create(address)` لبدء فهرسة عقد التبادل الجديد.
...
تسمح سياقات مصدر البيانات بتمرير تكوين (configuration) إضافي عند عمل instantiating للقالب. في مثالنا ، لنفترض أن التبادلات مرتبطة بزوج تداول معين ، والذي تم تضمينه في حدث `NewExchange`. يمكن تمرير هذه المعلومات إلى مصدر البيانات الذي تم إنشاء instance له ، مثل:
...
يعد `startBlock` إعدادا اختياريا يسمح لك بتحديد كتلة في السلسلة والتي سيبدأ مصدر البيانات بالفهرسة. وتعيين كتلة البدء يسمح لمصدر البيانات بتخطي الملايين من الكتل التي ربما ليست ذات صلة. عادةً ما يقوم مطور الـ subgraph بتعيين `startBlock` إلى الكتلة التي تم فيها إنشاء العقد الذكي لمصدر البيانات.
...
بينما توفر الأحداث طريقة فعالة لجمع التغييرات ذات الصلة بحالة العقد ، تتجنب العديد من العقود إنشاء سجلات لتحسين تكاليف الغاز. في هذه الحالات ، يمكن لـ subgraph الاشتراك في الاستدعاء الذي يتم إجراؤه على عقد مصدر البيانات. ويتم تحقيق ذلك من خلال تعريف معالجات الاستدعاء التي تشير إلى توقيع (signature) الدالة ومعالج الـ mapping الذي سيعالج الاستدعاءات لهذه الدالة. لمعالجة هذه الاستدعاءات، سيتلقى معالج الـ mapping الـ`ethereum.Call` كـ argument مع المدخلات المكتوبة والمخرجات من الاستدعاء. وستؤدي الاستدعاءات التي يتم إجراؤها على أي عمق في سلسلة استدعاء الاجراء إلى تشغيل الـ mapping، مما يسمح بالتقاط النشاط مع عقد مصدر البيانات من خلال عقود الـ proxy.
...
الـ `function` هي توقيع الدالة المعياري لفلترة الاستدعاءات من خلالها. وخاصية `handler` هي اسم الدالة في الـ mapping الذي ترغب في تنفيذه عندما يتم استدعاء الدالة المستهدفة في عقد مصدر البيانات.
...
إذا كنت بحاجة إلى معالجة أحداث anonymous في Solidity ، فيمكن تحقيق ذلك من خلال توفير topic0 للحدث ، كما في المثال:
...
سيتم تشغيل حدث فقط عندما يتطابق كل من التوقيع و topic0. بشكل افتراضي ، `topic0` يساوي hash توقيع الحدث.
...
لاحظ أن استخدام ميزة بدون الإعلان عنها سيؤدي إلى حدوث **validation error** أثناء نشر الـ subgraph ، ولكن لن تحدث أخطاء إذا تم الإعلان عن الميزة ولكن لم يتم استخدامها.

### IPFS على عقود Ethereum

حالة الاستخدام الشائعة لدمج IPFS مع Ethereum هي تخزين البيانات على IPFS التي ستكون مكلفة للغاية للحفاظ عليها في السلسلة ، وتشير إلى IPFS hash في عقود Ethereum.
...
يجب أن يتضمن الاستعلام أيضا الاستعلام عن البيانات ذات التناقضات المحتملة من خلال الوسيطة `subgraphError`. ويوصى أيضا بالاستعلام عن `meta_` للتحقق مما إذا كان الـ subgraph قد تخطى الأخطاء ، كما في المثال:
...
لأن تطعيم (grafting) النُسخ بدلا من فهرسة البيانات الأساسية قإنه يكون أسرع بكثير في الحصول على subgraph للكتلة المطلوبة من الفهرسة من نقطة الصفر ، على الرغم من أن نسخة البيانات الأولية يمكن أن تستغرق عدة ساعات لـ subgraphs كبيرة جدا. أثناء بدء الـ grafted subgraph ، سيقوم Graph Node بتسجيل المعلومات حول أنواع الكيانات التي تم نسخها بالفعل.
