...
- `dataSources.mapping.eventHandlers`: يضع قائمة بأحداث العقد الذكية التي يتفاعل معها هذا الـ subgraph ويعالجها في الـتخطيط —./src/mapping.ts في المثال - التي تحول هذه الأحداث إلى كيانات في المخزن.

- `dataSources.mapping.callHandlers`: يضع قائمة بدوال العقد الذكي التي يتفاعل معها هذا الـ subgraph ويعالجها في الـ mapping التي تحول المدخلات والمخرجات لاستدعاءات الدالة إلى كيانات في المخزن.

- `dataSources.mapping.blockHandlers`: يضع قائمة بـالكتل التي يتفاعل معها هذا الـ subgraph ويعالجها في الـ mapping لتشغيلها عند إلحاق كتلة بالسلسلة.. بدون فلتر، سيتم تشغيل معالج الكتلة في كل كتلة. يمكن توفير فلتر اختياري مع الأنواع التالية:call. سيقوم فلتر `call بتشغيل المعالج إذا كانت الكتلة تحتوي على استدعاء واحد على الأقل لعقد مصدر البيانات.
...
لأن grafting النُسخ بدلا من فهرسة البيانات الأساسية قإنه يكون أسرع بكثير في الحصول على subgraph للكتلة المطلوبة من الفهرسة من نقطة الصفر ، على الرغم من أن نسخة البيانات الأولية يمكن أن تستغرق عدة ساعات لـ subgraphs كبيرة جدا. أثناء تهيئة الـ grafted subgraph ، سيقوم Graph Node بتسجيل المعلومات حول أنواع الكيانات التي تم نسخها بالفعل.
