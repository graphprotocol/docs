...
اتبع أولاً التعليمات [ هنا ](/developing/defining-a-subgraph) لتثبيت Graph CLI. قم بإنشاء subgraph عن طريق تمرير `graph init --product hosted-service`
...
يعتمد مثال الـ subgraph على عقد Gravity بواسطة Dani Grant الذي يدير avatars للمستخدم ويصدر أحداث `NewGravatar` أو `UpdateGravatar` كلما تم إنشاء avatars أو تحديثها. حيث يعالج الـ subgraph هذه الأحداث عن طريق كتابة كيانات `Gravatar` إلى مخزن Graph Node والتأكد من تحديثها وفقا للأحداث. انتقل إلى [subgraph manifest](/developing/creating-a-subgraph#the-subgraph-manifest) لفهم أفضل لـ mappings والأحداث من عقودك الذكية التي يجب الانتباه إليها و والمزيد.
